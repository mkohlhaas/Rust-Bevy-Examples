...
pub fn extract_lights(
  ...
  ) in spot_lights.iter()
  {
    ...
    let extracted_spot_light = ExtractedPointLight {
      ...
      // The factor of SQRT_2 is for the worst-case diagonal offset
      shadow_normal_bias: spot_light.shadow_normal_bias * texel_size * core::f32::consts::SQRT_2,
      ...
      for (main_auxiliary_entity, visible_mesh_entities_list) in visible_entities.entities.iter() {
        for subview_index in 0..(cascade_config.bounds.len() as u32) {
          ...
          let extracted_entities = &mut existing_extracted_shadow_map_visible_entities
            ...
            .entities;
          extracted_entities.clear();
          let Some(visible_mesh_entities) = visible_mesh_entities_list.get(subview_index as usize)
          else {
            continue;
          };
          extracted_entities.extend(visible_mesh_entities.entities.iter().map(|main_entity| {
...
pub fn prepare_lights(
  ...
  if !*max_cascades_per_light_warning_emitted
    && directional_lights
      .iter()
      .any(|(_, _, light, _)| light.cascade_shadow_config.bounds.len() > MAX_CASCADES_PER_LIGHT)
  {
    warn!(
  ...
  let spot_light_shadow_maps_count = point_lights
    .iter()
    .filter(|(_, _, light, _, _, _)| light.shadow_maps_enabled && light.spot_light_angles.is_some())
    .count()
    .min(max_texture_array_layers - directional_shadow_enabled_count * MAX_CASCADES_PER_LIGHT);
    ...
    .take(point_light_count.min(max_texture_cubes))
  {
    ...
    for face_index in 0..6 {
      let retained_view_entity = RetainedViewEntity::new(*light_main_entity, None, face_index);
    ...
    .filter_map(|sorted_camera| views.get(sorted_camera.entity).ok())
  {
    ...
    for light_entity in directional_light_entities.iter().filter(|light_entity| {
      ...
      let Ok(mut light_view_entities) = directional_light_view_entities.get_mut(*light_entity)
      else {
        continue;
      };
      ...
      .take(MAX_DIRECTIONAL_LIGHTS)
    {
        ...
        iter.zip(light_view_entities.iter().copied()).enumerate()
      {
        ...
        commands.entity(view_light_entity).insert((
          ShadowView {
            depth_attachment,
            pass_name: format!("shadow_directional_light_{light_index}_cascade_{cascade_index}"),
          },
          ExtractedView {
            retained_view_entity,
            viewport: UVec4::new(
  ...
  for mut entities in &mut directional_light_view_entities {
    for (_, light_view_entities) in entities.extract_if(|entity, _| !live_views.contains(entity)) {
...
fn create_point_shadow_maps(
  ...
  let cube_face_projection =
    proj::perspective_infinite_reverse(core::f32::consts::FRAC_PI_2, 1.0, light.shadow_map_near_z);
...
pub fn check_views_lights_need_specialization(
  ...
  for (light_entity, extracted_view_light) in &view_light_entities {
    ...
    if let Some(current_key) = light_key_cache.get_mut(&extracted_view_light.retained_view_entity) {
...
pub(crate) fn specialize_shadows(
...

  {
    ...
    for (light_entity, extracted_view_light) in &view_light_entities {
      ...
      let Some(light_key) = light_key_cache.get(&extracted_view_light.retained_view_entity) else {
        continue;
      };
      ...
      let view_pending_shadow_queues =
        pending_shadow_queues.prepare_for_new_frame(extracted_view_light.retained_view_entity);
      ...
      for (render_entity, visible_entity) in dirty_specializations.iter_to_specialize(
        ...
        let mut mesh_key = *light_key | MeshPipelineKey::from_bits_retain(mesh.key_bits.bits());
