...
impl Stepping {
  ...
  fn next_frame(&mut self) {
    ...
    for update in self.updates.drain(..) {
      match update {
        ...
        Update::ClearBehavior(label, system) => match self.schedule_states.get_mut(&label) {
  ...
  pub fn skipped_systems(&mut self, schedule: &Schedule) -> Option<FixedBitSet> {
    ...
    let (skip_list, next_system) = if index == cursor.schedule {
      let (skip_list, next_system) = state.skipped_systems(schedule, cursor.system, self.action);
