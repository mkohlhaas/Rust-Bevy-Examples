...
impl ScheduleData {
  ...
  pub fn from_schedule(
    ...
    let systems = schedule
      .systems_with_access()
      .map_err(|_| ExtractAppDataError::ScheduleNotInitialized(format!("{:?}", schedule.label())))?
      .enumerate()
      .map(|(index, (key, system_with_access))| {
        ...
        SystemData {
          ...
          apply_deferred: system.system_type() == core::any::TypeId::of::<ApplyDeferred>(),
    ...
    let node_id_to_schedule_index = |node_id: NodeId| match node_id {
      NodeId::System(key) => ScheduleIndex::System(
        *system_key_to_index
          .get(&key)
          .expect("the system this key refers to should have already been seen") as _,
      ),
      NodeId::Set(key) => ScheduleIndex::SystemSet(
        *system_set_key_to_index
          .get(&key)
          .expect("the system set this key refers to should have already been seen") as _,
      ),
    };
      ...
      .map(|(system_1, system_2, conflicts)| {
        ...
        SystemConflict {
          ...
          conflicting_access: if conflicts.is_empty() {
            ...
            AccessConflict::Components(component_trace.get_indexes(conflicts.iter().copied()))
          },
        }
      })
      .collect();
...
pub mod tests {
  ...
  fn stack_of_system_sets() {
    ...
    app
      .add_systems(Update, a.in_set(MySet::<0>))
