use crate::{
  ComputedNode, ComputedUiRenderTargetInfo, ContentSize, FixedMeasure, Measure, MeasureArgs, Node,
  NodeMeasure,
};
...
impl TextMeasure {
  ...
  pub const fn needs_buffer(
    ...
    height.is_none() && (width.is_some() || matches!(available_width, AvailableSpace::Definite(_)))
  }
}
...
pub fn measure_text_system(
  ...
  ) in &mut text_query
  {
    ...
    match text_pipeline.create_text_measure(
      ...
      Err(
        TextError::NoSuchFont | TextError::NoSuchFontFamily(_) | TextError::DegenerateScaleFactor,
      ) => {
...
pub fn text_system(
  ...
  for (node, block, mut text_layout_info, mut text_flags, mut computed, hinting) in &mut text_query
  {
