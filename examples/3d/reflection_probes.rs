...
impl AppStatus {
  ...
  fn create_text(&self) -> Text {
    ...
    format!(
      ...
      self.reflection_mode, rotation_help_text, self.sphere_roughness, REFLECTION_MODE_HELP_TEXT
    )
    .into()
  }
}
...
impl FromWorld for Cubemaps {
  fn from_world(world: &mut World) -> Self {
    Cubemaps {
      diffuse_environment_map: world.load_asset("environment_maps/pisa_diffuse_rgb9e5_zstd.ktx2"),
      specular_environment_map: world.load_asset("environment_maps/pisa_specular_rgb9e5_zstd.ktx2"),
...
fn change_sphere_roughness(
  ...
  if roughness_delta != 0.0 {
    ...
    app_status.sphere_roughness = (app_status.sphere_roughness + roughness_delta).clamp(0.0, 1.0);
