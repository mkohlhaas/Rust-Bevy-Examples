...
use bevy::{
  ...
  render::{
    batching::{
      gpu_preprocessing::{
        batch_and_prepare_sorted_render_phase, BatchedInstanceBuffers, IndirectParametersMetadata,
    ...
    render_resource::{
      CachedRenderPipelineId, ColorTargetState, ColorWrites, Face, FragmentState, PipelineCache,
      PrimitiveState, RenderPassDescriptor, RenderPipelineDescriptor, SpecializedMeshPipeline,
...
fn queue_custom_meshes(
  ...
  for (view, visible_entities) in &mut views {
    ...
    for &main_entity in
      dirty_specializations.iter_to_dequeue(view.retained_view_entity, render_visible_mesh_entities)
    {
