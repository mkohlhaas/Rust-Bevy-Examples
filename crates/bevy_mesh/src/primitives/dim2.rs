...
impl MeshBuilder for CircularSectorMeshBuilder {
  fn build(&self) -> Mesh {
    ...
    for i in 0..self.resolution {
      ...
      let uv = Vec2::from_angle(-(angle + uv_angle)).mul_add(Vec2::splat(0.5), Vec2::splat(0.5));
  ...
  P: Primitive2d + Meshable,
{
  ...
  fn get_vertex_attributes(&self) -> Option<RingMeshBuilderVertexAttributes> {
    ...
    fn get_uvs(mesh: &mut Mesh) -> Option<&mut Vec<[f32; 2]>> {
      if let VertexAttributeValues::Float32x2(data) = mesh.attribute_mut(Mesh::ATTRIBUTE_UV_0)? {
        Some(data)
      } else {
        None
      }
    ...
    fn get_normals(mesh: &mut Mesh) -> Option<&mut Vec<[f32; 3]>> {
      if let VertexAttributeValues::Float32x3(data) = mesh.attribute_mut(Mesh::ATTRIBUTE_NORMAL)? {
        Some(data)
      } else {
        None
      }
