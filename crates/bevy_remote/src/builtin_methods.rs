...
pub fn process_remote_get_resources_request(
  ...
  let value = serialized_object
    .into_values()
    .next()
    .ok_or_else(|| BrpError::internal(anyhow!("Unexpected format of serialized resource value")))?;
...
pub fn process_remote_get_components_watching_request(
  ...
  'component_loop: for component_path in components {
    let Ok(type_registration) = get_component_type_registration(&type_registry, &component_path)
    else {
      let err = BrpError::component_error(format!("Unknown component type: `{component_path}`"));
      if strict {
        return Err(err);
      }
      errors.insert(
    ...
    let Some(component_id) = world.components().get_valid_id(type_registration.type_id()) else {
...
pub fn process_remote_mutate_components_request(
  ...
  let component_type: &TypeRegistration = type_registry
    ...
    .ok_or_else(|| BrpError::component_error(anyhow!("Unknown component type: `{}`", component)))?;
...
pub fn process_remote_trigger_event_request(
  ...
  world.resource_scope(|world, registry: Mut<AppTypeRegistry>| {
    ...
    if let Some(payload) = value {
      let payload: Box<dyn PartialReflect> = TypedReflectDeserializer::new(registration, &registry)
...
pub fn process_remote_write_message_request(
  ...
  world.resource_scope(|world, registry: Mut<AppTypeRegistry>| {
    ...
    if let Some(payload) = value {
      let payload: Box<dyn PartialReflect> = TypedReflectDeserializer::new(registration, &registry)
        .deserialize(payload.into_deserializer())
        .map_err(|err| BrpError::resource_error(format!("{message} is invalid: {err}")))?;
  ...
  let schemas = types
    ...
    .filter_map(|type_reg| {
      ...
      if let Some(crate_name) = &path_table.crate_name() {
        if !filter.with_crates.is_empty() && !filter.with_crates.iter().any(|c| crate_name.eq(c)) {
          return None;
        }
