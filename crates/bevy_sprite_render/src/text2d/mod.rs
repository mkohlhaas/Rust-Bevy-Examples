...
pub fn extract_text2d_sprite(
  ...
  ) in text2d_query.iter()
  {
    ...
    if let Some(shadow) = maybe_shadow {
      ...
      for run in text_layout_info.run_geometry.iter() {
        let section_entity = computed_block.entities()[run.section_index as usize].entity;
        let Ok((_, has_strikethrough, has_underline, _, _)) = decoration_query.get(section_entity)
        else {
          continue;
        };
        ...
        if has_strikethrough {
          ...
          let transform = shadow_transform * GlobalTransform::from_translation(offset.extend(0.));
          extracted_sprites.sprites.push(ExtractedSprite {
            main_entity,
            render_entity,
            transform,
    ...
    ) in text_layout_info.glyphs.iter().enumerate()
    {
      ...
      if text_layout_info.glyphs.get(i + 1).is_none_or(|info| {
        info.section_index != current_section || info.atlas_info.texture != atlas_info.texture
      }) {
        let render_entity = commands.spawn(TemporaryRenderEntity::default()).id();
        extracted_sprites.sprites.push(ExtractedSprite {
          main_entity,
