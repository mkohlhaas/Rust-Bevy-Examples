...
pub fn extract_shadows(
    ...
    .chain(removed_computed_ui_render_target_info_query.read())
  {
    ...
    let Some(mut extracted_nodes) = extracted_box_shadows.box_shadows.remove(&main_entity) else {
      continue;
    };
  ...
  if let Some(view_binding) = view_uniforms.uniforms.binding() {
    ...
    for ui_phase in phases.values_mut() {
      for item_index in 0..ui_phase.items.len() {
        ...
        *ui_phase.items[item_index].batch_range_mut() = item_index as u32..item_index as u32 + 1;
      }
    }
