...
pub struct ShaderCache<ShaderModule, RenderDevice> {
  ...
  load_module:
    fn(&RenderDevice, ShaderCacheSource, &ValidateShader) -> Result<ShaderModule, ShaderCacheError>,
            ...
            if let ShaderImport::AssetPath(path) = &shader.import_path {
              ...
              for shader_def in shader_defs {
                match shader_def {
                  ...
                  _ => {
                    debug!("ShaderDefVal::Int and ShaderDefVal::UInt are not supported in wesl",)
                  }
                }
              }
