...
impl Ease for Dir3A {
  fn interpolating_curve_unbounded(start: Self, end: Self) -> impl Curve<Self> {
    let difference_quat = Quat::from_rotation_arc(start.as_vec3a().into(), end.as_vec3a().into());
    Quat::interpolating_curve_unbounded(Quat::IDENTITY, difference_quat).map(move |q| q * start)
  }
}
    ...
    FunctionCurve::new(Interval::EVERYWHERE, move |t| {
      ...
      Isometry2d {
        rotation: Rot2::interpolating_curve_unbounded(start.rotation, end.rotation)
          .sample_unchecked(t),
        translation: crate::Vec2::interpolating_curve_unbounded(start.translation, end.translation)
          .sample_unchecked(t),
      }
    })
  }
...
impl EaseFunction {
  fn eval(&self, t: f32) -> f32 {
    match self {
      ...
      EaseFunction::Steps(num_steps, jump_at) => easing_functions::steps(*num_steps, *jump_at, t),
...
mod tests {
  ...
  fn ease_quats() {
    ...
    {
      let (before_mid_axis, before_mid_angle) = quat_curve.sample(0.25).unwrap().to_axis_angle();
