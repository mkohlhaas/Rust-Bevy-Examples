...
use self::{
  ...
  gltf_ext::{
    ...
    material::{
      alpha_mode, material_label, needs_tangents, uv_channel, warn_on_differing_texture_transforms,
    },
...
impl GltfLoader {
  ...
  pub async fn load_gltf<'a, 'b, 'c>(
      ...
      for animation in gltf.animations() {
        ...
        for channel in animation.channels() {
            ...
            match outputs {
                ...
                if keyframe_timestamps.len() == 1 {
                  ...
                  match interpolation {
                    gltf::animation::Interpolation::Linear => {
                      UnevenSampleAutoCurve::new(keyframe_timestamps.into_iter().zip(translations))
                        .ok()
                        .map(|curve| {
                          VariableCurve::new(AnimatableCurve::new(translation_property, curve))
                        })
                    }
                    gltf::animation::Interpolation::Step => {
                      SteppedKeyframeCurve::new(keyframe_timestamps.into_iter().zip(translations))
                        .ok()
                        .map(|curve| {
                          VariableCurve::new(AnimatableCurve::new(translation_property, curve))
                        })
              ...
              ReadOutputs::Rotations(rots) => {
                ...
                if keyframe_timestamps.len() == 1 {
                  ...
                  match interpolation {
                    gltf::animation::Interpolation::Linear => {
                      UnevenSampleAutoCurve::new(keyframe_timestamps.into_iter().zip(rotations))
                        .ok()
                        .map(|curve| {
                          VariableCurve::new(AnimatableCurve::new(rotation_property, curve))
                        })
                    }
                    gltf::animation::Interpolation::Step => {
                      SteppedKeyframeCurve::new(keyframe_timestamps.into_iter().zip(rotations))
                        .ok()
                        .map(|curve| {
                          VariableCurve::new(AnimatableCurve::new(rotation_property, curve))
                        })
                    }
                    gltf::animation::Interpolation::CubicSpline => CubicRotationCurve::new(
                      keyframe_timestamps,
              ...
              ReadOutputs::Scales(scale) => {
                ...
                if keyframe_timestamps.len() == 1 {
                  ...
                  match interpolation {
                    gltf::animation::Interpolation::Linear => {
                      UnevenSampleAutoCurve::new(keyframe_timestamps.into_iter().zip(scales))
                        .ok()
                        .map(|curve| {
                          VariableCurve::new(AnimatableCurve::new(scale_property, curve))
                        })
                    }
                    gltf::animation::Interpolation::Step => {
                      SteppedKeyframeCurve::new(keyframe_timestamps.into_iter().zip(scales))
                        .ok()
                        .map(|curve| {
                          VariableCurve::new(AnimatableCurve::new(scale_property, curve))
                        })
          ...
          if let Some((root_index, path)) = paths.get(&node.index()) {
            ...
            animation_clip
              .add_variable_curve_to_target(AnimationTargetId::from_names(path.iter()), curve);
          } else {
            warn!(
    ...
    for gltf_mesh in gltf.meshes() {
      ...
      let gltf_mesh_on_non_skinned_nodes = meshes_on_non_skinned_nodes.contains(&gltf_mesh.index());
      ...
      .map(|gltf_skin| {
        ...
        let local_to_bone_bind_matrices: Vec<Mat4> = reader
          ...
          .map(|mats| {
            mats
              .map(|mat| {
                Mat4::from_cols_array_2d(&mat) * convert_coordinates.mesh_conversion_mat4()
              })
              .collect()
          })
          .unwrap_or_else(|| {
      ...
      let skin = node.skin().map(|skin| {
        skins
          ...
          .or_insert_with(|| {
            ...
            let handle =
              load_context.add_labeled_asset(gltf_skin.asset_label().to_string(), gltf_skin);
    ...
    for scene in gltf.scenes() {
      ...
      for extension in extensions.iter_mut() {
        extension.on_scene_completed(&mut scene_load_context, &scene, world_root_id, &mut world);
      }
