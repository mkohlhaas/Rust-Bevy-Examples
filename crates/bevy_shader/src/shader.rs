...
impl From<&Source> for naga_oil::compose::ShaderLanguage {
  fn from(value: &Source) -> Self {
    match value {
      ...
      Source::Glsl(_, _) => {
        panic!("GLSL is not supported in this configuration; use the feature `shader_format_glsl`")
      }
      Source::SpirV(_) => panic!("spirv not yet implemented"),
...
impl AssetLoader for ShaderLoader {
  ...
  async fn load(
    ...
    let mut shader = match ext {
      ...
      "frag" => Shader::from_glsl(String::from_utf8(bytes)?, naga::ShaderStage::Fragment, path),
      "comp" => Shader::from_glsl(String::from_utf8(bytes)?, naga::ShaderStage::Compute, path),
      #[cfg(feature = "shader_format_wesl")]
