...
impl NumberInputValue {
  ...
  fn offset_by(self, delta: f64) -> Self {
    match self {
      ...
      NumberInputValue::I32(v) => NumberInputValue::I32(v.saturating_add(delta.round() as i32)),
      NumberInputValue::I64(v) => NumberInputValue::I64(v.saturating_add(delta.round() as i64)),
    }
  }
...
impl NumberInputRange {
  ...
  pub fn clamp(&self, n: NumberInputValue) -> NumberInputValue {
    match (self, n) {
      (Self::F32(r), NumberInputValue::F32(v)) => NumberInputValue::F32(v.clamp(r.start, r.end)),
      (Self::F64(r), NumberInputValue::F64(v)) => NumberInputValue::F64(v.clamp(r.start, r.end)),
      (Self::I32(r), NumberInputValue::I32(v)) => NumberInputValue::I32(v.clamp(r.start, r.end)),
      (Self::I64(r), NumberInputValue::I64(v)) => NumberInputValue::I64(v.clamp(r.start, r.end)),
      (range, value) => {
    ...
    && let Ok(disabled) = q_root.get(root_id)
  {
    ...
    if !disabled && drag_state.max_distance <= DRAG_THRESHOLD_DISTANCE {
      ...
      let Some(local_pos) = transform.try_inverse().map(|inverse| {
        inverse
          .transform_point2(release.pointer_location.position * target.scale_factor() / ui_scale.0)
