...
impl DynamicSet {
  ...
  fn internal_eq(value: &dyn PartialReflect) -> impl FnMut(&Box<dyn PartialReflect>) -> bool + '_ {
    |other| {
      value.reflect_partial_eq(&**other).expect(
  ...
  fn insert_boxed(&mut self, value: Box<dyn PartialReflect>) -> bool {
      ...
      .find_mut(Self::internal_hash(&*value), Self::internal_eq(&*value))
    {
      ...
      None => {
        self
          .hash_table
          .insert_unique(Self::internal_hash(value.as_ref()), value, |boxed| {
