...
impl HoveredEntityAncestors {
  ...
  pub fn rebuild(
      ...
      .flat_map(|(_, hashmap)| hashmap.iter().map(|data| *data.0))
    {
      ...
      if let Some(previous_entry) = pointer_state.hovered_entity_ancestors.get(&hovered_entity) {
...
pub fn pointer_events(
    ...
    .flat_map(|(id, hashmap)| hashmap.iter().map(|data| (*id, *data.0, data.1.clone())))
  {
      ...
      .any(|e| e.contains_key(&hovered_entity))
    {
      ...
      let mut entities_to_send_leave = pointer_state.get_ancestors(&hovered_entity).map_or_else(
        || {
          ancestors_query
    ...
    .flat_map(|(id, hashmap)| hashmap.iter().map(|data| (*id, *data.0, data.1.clone())))
  {
      ...
      .any(|e| e.contains_key(&hovered_entity))
    {
      ...
      // Ensure we do not double send to any other entities that have already been sent to during this loop
      entities_to_send_enter.retain(|entity: &Entity| !sent_enter.contains(&(pointer_id, *entity)));
  ...
  } in input_events.read().cloned()
  {
    match action {
      PointerAction::Press(button) => {
        ...
        state
          .clicking
          .retain(|_, (last_click, _)| now - *last_click <= picking_settings.multi_click_interval);
...
mod tests {
  ...
  fn enter_leave_events() {
    ...
    app
      .init_resource::<EnterEventCounts>()
