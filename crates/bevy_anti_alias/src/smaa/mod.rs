...
use bevy_render::{
  ...
  render_resource::{
    ...
    StencilOperation, StencilState, StoreOp, TextureDescriptor, TextureDimension, TextureFormat,
  ...
  for (entity, smaa_textures) in &view_targets {
    ...
    commands.entity(entity).insert(SmaaBindGroups {
      ...
      neighborhood_blending_bind_group: render_device.create_bind_group(
        ...
        &BindGroupEntries::sequential((&smaa_textures.blend_texture.default_view, &sampler)),
      ),
    });
  }
}
