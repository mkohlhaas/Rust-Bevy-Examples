  ...
  fn prepare<'a>(&self, sh: &'a xshell::Shell) -> Vec<PreparedCommand<'a>> {
    ...
    match &self.command {
      ...
      None => {
        ...
        cmds.append(&mut commands::IntegrationTestCheckCommand::default().prepare(sh, args));
        cmds.append(&mut commands::IntegrationTestCleanCommand::default().prepare(sh, args));
