...
pub fn bloom(
  ...
  let downsampling_first_bind_group = ctx.render_device().create_bind_group(
    ...
    &BindGroupEntries::sequential((view_texture, &bind_groups.sampler, uniforms_binding.clone())),
  );
  ...
  // First downsample pass
  {
    ...
    let mut downsampling_first_pass = command_encoder.begin_render_pass(&RenderPassDescriptor {
...
fn compute_blend_factor(bloom: &Bloom, mip: f32, max_mip: f32) -> f32 {
  ...
  let high_pass_lq = 1.0
    - (((mip / max_mip) - bloom.high_pass_frequency) / bloom.high_pass_frequency).clamp(0.0, 1.0);
