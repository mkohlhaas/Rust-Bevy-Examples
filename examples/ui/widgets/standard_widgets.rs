  ...
  (
    ...
    children![
      ...
      (
        ...
        observe(
          |value_change: On<ValueChange<f32>>, mut widget_states: ResMut<DemoWidgetStates>| {
...
fn update_slider_style2(
  sliders: Query<(Entity, &Hovered, &SliderDragState, Has<InteractionDisabled>), With<DemoSlider>>,
  children: Query<&Children>,
...
fn update_checkbox_or_radio_style2(
  ...
  removed_checked
    ...
    .chain(removed_disabled.read())
    .for_each(|entity| {
      if let Ok((checked, Hovered(is_hovering), is_disabled, children)) = q_checkbox.get_mut(entity)
      {
        let Some(border_id) = children.first() else {
          return;
        };
