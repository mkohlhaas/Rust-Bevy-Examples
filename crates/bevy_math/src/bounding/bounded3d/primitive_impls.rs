  ...
  fn aabb_3d(&self, isometry: impl Into<Isometry3d>) -> Aabb3d {
    ...
    Aabb3d {
      min: isometry.translation
        + (top - self.radius_top * half_extents).min(bottom - self.radius_bottom * half_extents),
      max: isometry.translation
        + (top + self.radius_top * half_extents).max(bottom + self.radius_bottom * half_extents),
    }
  }
