...
fn spawn_main_camera(
  ...
  let camera_transform =
    Transform::from_translation(vec3(-2.0, 1.0, -2.0).normalize_or_zero() * CAMERA_ORBIT_DISTANCE)
...
fn play_fox_animation(
  mut commands: Commands,
  mut animation_players_query: Query<(Entity, &mut AnimationPlayer), Without<AnimationGraphHandle>>,
  asset_server: Res<AssetServer>,
  ...
  let (fox_animation_graph, fox_animation_node) = AnimationGraph::from_clip(fox_animation.clone());
