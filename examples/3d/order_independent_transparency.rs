  ...
  commands
    ...
    .observe(
      |event: On<Pointer<Drag>>,
       mut commands: Commands,
       mut camera_transforms: Single<(&mut Transform, &BaseTransform), With<Camera3d>>| {
        commands
          .entity(event.entity)
    ...
    .observe(
      |_: On<Pointer<DragStart>>,
       ...
       camera: Single<(Entity, &Transform), With<Camera3d>>| {
        ...
        commands.entity(camera).insert(BaseTransform(*transform));
      },
    )
    .observe(
...
fn spawn_quads(commands: &mut Commands, resources: &mut SceneResources) {
  ...
  let render_layers = RenderLayers::layer(1);
  let xform = |x, y, z| {
    Transform::from_rotation(Quat::from_rotation_y(0.5)).mul_transform(Transform::from_xyz(x, y, z))
  };
