...
use bevy_render::{
  ...
  render_resource::{
    ...
    SpecializedRenderPipeline, SpecializedRenderPipelines, TextureDimension, TextureFormat,
    TextureSampleType,
  },
...
impl Plugin for EffectStackPlugin {
  fn build(&self, app: &mut App) {
    ...
    app
      .add_plugins(ExtractComponentPlugin::<ChromaticAberration>::default())
...
pub(crate) fn prepare_post_processing_uniforms(
    ...
    views.iter_mut()
  {
    let chromatic_aberration_uniform_buffer_offset =
      if let Some(chromatic_aberration) = maybe_chromatic_aberration {
        post_processing_uniform_buffers
          .chromatic_aberration
          .push(&ChromaticAberrationUniform {
    ...
    let lens_distortion_uniform_buffer_offset = if let Some(lens_distortion) = maybe_lens_distortion
    {
      post_processing_uniform_buffers
        .lens_distortion
