...
impl<M: Message> MessageCursor<M> {
  ...
  pub fn read_with_id<'a>(&'a mut self, messages: &'a Messages<M>) -> MessageIteratorWithId<'a, M> {
  ...
  #[cfg(feature = "multi_threaded")]
  pub fn par_read_mut<'a>(&'a mut self, messages: &'a mut Messages<M>) -> MessageMutParIter<'a, M> {
