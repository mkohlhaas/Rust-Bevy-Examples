      ...
      if let std::backtrace::BacktraceStatus::Captured = backtrace.status() {
        ...
        for line in backtrace_str.split('\n') {
          if !full_backtrace {
            ...
            if line
              .contains("<bevy_ecs::error::bevy_error::BevyError as core::convert::From<E>>::from")
            {
              skip_next_location_line = true;
              continue;
            }
...
mod tests {
  ...
  fn context() {
    ...
    let err: Result<i32, BevyError> = Err(BevyError::new(crate::error::Severity::Debug, "Oh no!"));
