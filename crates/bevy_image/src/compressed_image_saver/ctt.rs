...
use ctt::{
  convert, ColorSpace, Container, ConvertSettings, Image as CttImage, MipmapFilter, PipelineOutput,
  Quality, Surface, TextureKind,
};

use super::{
  ctt_helpers::{bevy_to_ctt_alpha_mode, choose_ctt_compressed_format, wgpu_to_ctt_texture_format},
...
impl CompressedImageSaverCtt {
  pub async fn save(
    ...
    let bytes_per_pixel =
      crate::TextureFormatPixelInfo::pixel_size(&image.texture_descriptor.format).map_err(|_| {
        CompressedImageSaverError::UnsupportedFormat(image.texture_descriptor.format)
      })? as u32;
