...
pub mod prelude {
  #[doc(hidden)]
  pub use crate::{
    animatable::*, animation_curves::*, graph::*, transition::*, AnimationClip, AnimationPlayer,
...
impl AnimationClip {
  ...
  #[inline]
  pub fn curves_for_target(&self, target_id: AnimationTargetId) -> Option<&'_ Vec<VariableCurve>> {
  ...
  pub(crate) fn get_or_insert_with(
    ...
    match id {
      ...
      EvaluatorId::Type(type_id) => match self.type_id_curve_evaluators.entry(type_id) {
        ...
        bevy_utils::TypeIdHashMapEntry::Vacant(vacant_entry) => &mut **vacant_entry.insert(func()),
      },
    }
  }
}
...
pub fn animate_targets(
  ...
  targets
    ...
    .for_each(|(entity, &target_id, &AnimatedBy(player_id), entity_mut)| {
      ...
      let Some(threaded_animation_graph) = threaded_animation_graphs.0.get(&animation_graph_id)
      else {
        return;
      };
      ...
      for &animation_graph_node_index in threaded_animation_graph.threaded_graph.iter() {
        let Some(animation_graph_node) = animation_graph.get(animation_graph_node_index) else {
          continue;
        };
        ...
        match animation_graph_node.node_type {
          AnimationNodeType::Blend => {
            ...
            .clone()
            {
              if let Err(err) = evaluation_state
                .blend_all(threaded_animation_graph.sorted_edges[edge_index as usize])
              {
            ...
            if let Err(err) = evaluation_state
              .push_blend_register_all(animation_graph_node.weight, animation_graph_node_index)
            {
          ...
          AnimationNodeType::Add => {
            ...
            {
              if let Err(err) =
                evaluation_state.add_all(threaded_animation_graph.sorted_edges[edge_index as usize])
              {
                warn!("Failed to blend animation: {:?}", err);
              }
            }
          ...
          AnimationNodeType::Clip(ref animation_clip_handle) => {
            ...
            if active_animation.weight == 0.0
              || (target_mask
                & threaded_animation_graph.computed_masks[animation_graph_node_index.index()])
                != 0
            {
              continue;
            }
...
impl AnimationEvaluationState {
  ...
  fn blend_all(&mut self, node_index: AnimationNodeIndex) -> Result<(), AnimationEvaluationError> {
...
impl<'a> Iterator for TriggeredEventsIter<'a> {
  ...
  fn next(&mut self) -> Option<Self::Item> {
    match self {
      ...
      TriggeredEventsIter::ForwardLooping { upper, lower } => upper.next().or_else(|| lower.next()),
      TriggeredEventsIter::ReverseLooping { lower, upper } => lower.next().or_else(|| upper.next()),
    }
  }
}
...
mod tests {
  ...
  fn test_sample_at_time() {
    ...
    let animatable_curve = AnimatableCurve::new(
      animated_field!(Transform::translation),
      AnimatableKeyframeCurve::new([(0.0, Vec3::new(0., 0., 1.)), (1.0, Vec3::new(1., 0., 0.))])
        .expect("Failed to create power level curve"),
    );
    let target_1 = AnimationTargetId::from_name(&Name::new("Target 1"));
