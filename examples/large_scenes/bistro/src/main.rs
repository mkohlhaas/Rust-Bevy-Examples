...
use mipmap_generator::{
  generate_mipmaps, MipmapGeneratorDebugTextPlugin, MipmapGeneratorPlugin, MipmapGeneratorSettings,
};
...
pub fn setup(mut commands: Commands, asset_server: Res<AssetServer>, args: Res<Args>) {
  ...
  let mut cam = commands.spawn((
    ...
    EnvironmentMapLight {
      ...
      specular_map: asset_server.load("environment_maps/san_giuseppe_bridge_4k_specular.ktx2"),
      intensity: 600.0,
      ..default()
    },
  ...
  cam
    .insert_if(DepthPrepass, || args.deferred)
...
fn frame_time_system(
  ...
  if let Some(frame_time) = diagnostics.get(&FrameTimeDiagnosticsPlugin::FRAME_TIME) {
    ...
    if frame_time.history_len() >= 100 {
      ...
      low_high.one_percent_high = measurements.iter().rev().take(count).sum::<f64>() / count as f64;
