...
use crate::{
  ...
  Justify, LetterSpacing, LineBreak, LineHeight, PositionedGlyph, TextBounds, TextEntity, TextFont,
  TextLayout,
};
  ...
  pub fn update_buffer<'a>(
    ...
    let result = {
        ...
        text_spans.enumerate()
      {
        ...
        if font_size <= 0.0 {
          warn_once!("Text span {entity} has a font size <= 0.0. Nothing will be displayed.");
          continue;
        }
      ...
      for item in line.items() {
        if let PositionedLayoutItem::GlyphRun(glyph_run) = item {
          ...
          for glyph in glyph_run.positioned_glyphs() {
            ...
            let atlas_info =
              match get_glyph_atlas_info(font_atlases, crate::GlyphCacheKey { glyph_id }) {
                Some(info) => info,
                None => {
