...
impl Segment2d {
  ...
  pub fn direction(&self) -> Dir2 {
    self
      .try_direction()
      .unwrap_or_else(|err| panic!("Failed to compute the direction of a line segment: {err}"))
  }
...
mod tests {
  ...
  fn segment_closest_point() {
    assert_eq!(
      Segment2d::new(Vec2::new(0.0, 0.0), Vec2::new(3.0, 0.0)).closest_point(Vec2::new(1.0, 6.0)),
      Vec2::new(1.0, 0.0)
    );
  ...
  fn triangle_math() {
    ...
    let acute_triangle = Triangle2d::new(Vec2::new(-1., 0.), Vec2::new(1., 0.), Vec2::new(0., 5.));
    let obtuse_triangle =
  ...
  fn regular_polygon_vertices() {
    ...
    assert!(
      (rotated_vertices.next().unwrap() - Vec2::new(-side_distance, side_distance)).length() < 1e-7,
    );
  }
}
