...
impl<'a, 'b> ComponentCloneCtx<'a, 'b> {
  ...
  pub fn write_target_component_reflect(&mut self, component: Box<dyn bevy_reflect::Reflect>) {
    ...
    let target_component_data_ptr = self.bundle_scratch_allocator.alloc_layout(component_layout);
    // SAFETY:
          ...
          for &component_id in bundle {
            let Some(component_ptr) = sparse_sets
              ...
              .or_else(|| {
                ...
                table
                  .as_mut()
                  .and_then(|table| unsafe { table.get_component(component_id, table_row) })
              })
            else {
...
impl<'w> EntityClonerBuilder<'w, OptIn> {
  ...
  fn allow_by_ids_inner<M: Marker>(&mut self, ids: impl FilterableIds<M>, insert_mode: InsertMode) {
  ...
  fn clone_components<'a>(
    ...
    for (&component, explicit) in self.allow.iter() {
      ...
      let do_clone = source_archetype.contains(component)
        && (explicit.insert_mode == InsertMode::Replace || !target_archetype.contains(component));
      if do_clone {
        clone_component(component);
