...
impl ExecutorState {
  ...
  fn can_run(&mut self, system_index: usize, conditions: &mut Conditions) -> bool {
    ...
    for set_idx in
      conditions.sets_with_conditions_of_systems[system_index].difference(&self.evaluated_sets)
    {
...
mod tests {
  ...
  use crate::{
    error::{BevyError, ErrorContext, FallbackErrorHandler, PANIC_ORIGINATES_FROM_ERROR_HANDLER},
    prelude::Resource,
