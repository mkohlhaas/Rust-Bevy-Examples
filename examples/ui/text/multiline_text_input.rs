      ...
      parent
        ...
        .with_children(|parent| {
            ...
            .observe(
              |on: On<FocusedInput<KeyboardInput>>,
               mut query_set: ParamSet<(
                ...
                let Ok(input) = visible_lines_query.get(on.original_event_target()) else {
                  return;
                };
          ...
          parent
            ...
            .observe(
              |on: On<FocusedInput<KeyboardInput>>,
               radius_input_query: Query<&EditableText, With<SelectionRadiusInput>>,
               mut cursor_style: Single<&mut TextCursorStyle, With<MultilineInput>>| {
                if !(on.input.state.is_pressed() && on.input.logical_key == Key::Enter) {
                  return;
                }

                let Ok(input) = radius_input_query.get(on.original_event_target()) else {
                  return;
                };
          ...
          parent
            ...
            .observe(
              |on: On<MenuEvent>,
               mut popup: Single<(&mut Node, &mut MenuFocusState), With<MenuPopup>>,
