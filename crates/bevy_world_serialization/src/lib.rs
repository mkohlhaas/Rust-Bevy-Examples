...
pub mod prelude {
  #[doc(hidden)]
  pub use crate::{
    DynamicWorld, DynamicWorldBuilder, DynamicWorldRoot, WorldAsset, WorldAssetRoot, WorldFilter,
  ...
  fn build(&self, app: &mut App) {
    ...
    app
      ...
      .on_remove(|mut world, context| {
        ...
        let id = handle.id();
        if let Some(&WorldInstance(instance_id)) = world.get::<WorldInstance>(context.entity) {
          let Some(mut world_instance_spawner) = world.get_resource_mut::<WorldInstanceSpawner>()
          else {
            return;
          };
          if let Some(instance_ids) = world_instance_spawner.spawned_dynamic_worlds.get_mut(&id) {
...
mod tests {
  ...
  fn world_instance_spawns_and_respawns_after_change() {
    ...
    app
      .add_plugins((AssetPlugin::default(), WorldSerializationPlugin))
    ...
    let (rectangle, _, child_of) = finish_line.components::<(&Rectangle, &FinishLine, &ChildOf)>();
    assert_eq!(
      rectangle,
      &Rectangle {
        width: 10.0,
