...
use crate::{
  ...
  schedule::{
    graph::{
      DagCrossDependencyError, DagOverlappingGroupError, DagRedundancyError, DiGraphToposortError,
      GraphNodeId,
    },
...
impl ScheduleBuildError {
  ...
  pub fn to_string(&self, graph: &ScheduleGraph, world: &World) -> String {
    match self {
      ...
      ScheduleBuildError::CrossDependency(error) => Self::cross_dependency_to_string(error, graph),
