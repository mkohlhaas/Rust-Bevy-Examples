  ...
  Clear: 'static + Send + Sync,
{
  fn drop(&mut self) {
    ...
    let positions =
      ellipse_inner(self.half_size, self.resolution).map(|vec2| self.isometry * vec2.extend(0.));
  ...
  Clear: 'static + Send + Sync,
{
  ...
  fn drop(&mut self) {
    ...
    let positions = ellipse_inner(self.half_size, self.resolution).map(|vec2| self.isometry * vec2);
