...
impl GltfExtensionHandler for GltfExtensionHandlerAnimation {
  ...
  fn on_animation(
    ...
    if gltf_animation.name().is_some_and(|v| v == "Run") {
      ...
      animation_clip.add_event_to_target(AnimationTargetId::from_iter(front_right_foot), 0.5, Step);
      animation_clip.add_event_to_target(AnimationTargetId::from_iter(back_left_foot), 0.0, Step);
      animation_clip.add_event_to_target(
...
fn observe_on_step(
  ...
  for _ in 0..14 {
    ...
    commands.spawn((
      Particle {
        lifetime_timer: Timer::from_seconds(seeded_rng.0.random_range(0.2..0.6), TimerMode::Once),
        size,
