...
impl App {
    ...
    B::Error: 'static + Send + Sync,
  {
    while !self.exit {
      ...
      let (mode_state, mode_entries) = match self.mode {
        ...
        Mode::MigrationGuides => (&mut self.migration_guide_state, &mut self.migration_guides),
      };
  ...
  fn write_output(self) -> Result<()> {
    ...
    let mut file =
      fs::File::create(self.content_dir.join("merged_migration_guides.md")).into_diagnostic()?;
