    ...
    if let Some(render_app) = app.get_sub_app_mut(RenderApp) {
      render_app
        ...
        .configure_sets(
          ExtractSchedule,
          (
            DirtySpecializationSystems::Clear.before(DirtySpecializationSystems::CheckForChanges),
        ...
        .add_systems(
          ...
          (
            ...
            clear_dirty_wireframe_specializations.in_set(DirtySpecializationSystems::Clear),
...
fn normalize_bgra8(target: &NormalizedRenderTarget, format: TextureFormat) -> TextureFormat {
  if matches!(target, NormalizedRenderTarget::Window(_)) && format == TextureFormat::Bgra8UnormSrgb
  {
      ...
      Either::Right(
        render_view_visible_mesh_entities
          ...
          .chain(self.changed_renderables.iter().filter_map(|main_entity| {
            self
              .entity_pair_from_visible_main_entity(render_view_visible_mesh_entities, main_entity)
          })),
      )
    })
    .chain(
    ...
    (if self.must_wipe_specializations_for_view(view) {
      ...
      Either::Right(
        render_visible_mesh_entities
          ...
          .map(|(entity, main_entity)| (entity, main_entity))
          .chain(self.changed_renderables.iter().filter_map(|main_entity| {
            self.entity_pair_from_visible_main_entity(render_visible_mesh_entities, main_entity)
          })),
      )
    })
    .chain(
...
pub fn expire_specializations_for_views(
  ...
  dirty_specializations
    .views
    .retain(|retained_view_entity| all_live_retained_view_entities.contains(retained_view_entity));
}
