...
use bevy_core_pipeline::{
  oit::{
    OitBuffers, OrderIndependentTransparencySettings, OrderIndependentTransparencySettingsOffset,
  },
...
use crate::{
  contact_shadows::{ContactShadowsBuffer, ContactShadowsUniform, ViewContactShadowsUniformOffset},
  decal::{
    self,
    clustered::{DecalsBuffer, RenderClusteredDecals, RenderViewClusteredDecalBindGroupEntries},
  },
  ...
  Bluenoise, ExtractedAtmosphere, FogMeta, GlobalClusterableObjectMeta, GpuClusteredLights, GpuFog,
  GpuLights, LightMeta, LightProbesBuffer, LightProbesUniform, MeshPipeline, MeshPipelineKey,
  RenderViewLightProbes, ScreenSpaceAmbientOcclusionResources, ScreenSpaceReflectionsBuffer,
  ...
  if let (
    ...
    ) in &views
    {
      ...
      if let Some(view_fog_offset) = view_fog_offset {
        ...
        entries = entries.extend_with_indices(((13, fog_meta.gpu_fogs.binding().unwrap()),));
      }
      ...
      if let Some(view_contact_shadows_offset) = view_contact_shadows_offset {
        ...
        entries = entries.extend_with_indices(((16, contact_shadows_buffer.0.binding().unwrap()),));
      }
      ...
      entries = entries.extend_with_indices(((24, transmission_view), (25, transmission_sampler)));
