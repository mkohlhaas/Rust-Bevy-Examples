...
pub fn prepare_meshlet_per_frame_resources(
  ...
  for (view_entity, view, render_layers, (_, shadow_view)) in &views {
    ...
    let first_bvh_cull_count_front = render_device.create_buffer_with_data(&BufferInitDescriptor {
    ...
    let first_bvh_cull_count_back = render_device.create_buffer_with_data(&BufferInitDescriptor {
    ...
    let second_bvh_cull_count_back = render_device.create_buffer_with_data(&BufferInitDescriptor {
    ...
    let front_meshlet_cull_count = render_device.create_buffer_with_data(&BufferInitDescriptor {
    ...
    let back_meshlet_cull_count = render_device.create_buffer_with_data(&BufferInitDescriptor {
      label: Some("meshlet_back_meshlet_cull_count"),
      contents: bytemuck::bytes_of(&0u32),
      usage: BufferUsages::STORAGE,
    });
    let back_meshlet_cull_dispatch = render_device.create_buffer_with_data(&BufferInitDescriptor {
      label: Some("meshlet_back_meshlet_cull_dispatch"),
      contents: DispatchIndirectArgs { x: 0, y: 1, z: 1 }.as_bytes(),
      usage: BufferUsages::STORAGE | BufferUsages::INDIRECT,
    });

    let visibility_buffer_software_raster_indirect_args =
      render_device.create_buffer_with_data(&BufferInitDescriptor {
    ...
    let visibility_buffer_hardware_raster_indirect_args =
      render_device.create_buffer_with_data(&BufferInitDescriptor {
    ...
    let previous_depth_pyramid = match resource_manager.previous_depth_pyramids.get(&view_entity) {
    ...
    commands.entity(view_entity).insert(MeshletViewResources {
      ...
      material_depth: not_shadow_view.then(|| texture_cache.get(&render_device, material_depth)),
  ...
  for (view_entity, view_resources) in &views {
    ...
    let resolve_material_depth = view_resources.material_depth.as_ref().map(|_| {
      render_device.create_bind_group(
        ...
        &pipeline_cache
          .get_bind_group_layout(&resource_manager.resolve_material_depth_bind_group_layout),
        &BindGroupEntries::sequential((
          &view_resources.visibility_buffer.default_view,
          resource_manager
            .visibility_buffer_raster_clusters
