...
use bevy::{
  ...
  render::{
    ...
    render_resource::{
      BufferUsages, Canonical, ColorTargetState, ColorWrites, CompareFunction, DepthStencilState,
      FragmentState, IndexFormat, PipelineCache, RawBufferVec, RenderPipeline,
...
fn queue_custom_phase_item(
  ...
  for (view, view_visible_entities, msaa) in views.iter() {
    ...
    let Some(render_visible_mesh_entities) = view_visible_entities.get::<CustomRenderedEntity>()
    else {
      continue;
    };
