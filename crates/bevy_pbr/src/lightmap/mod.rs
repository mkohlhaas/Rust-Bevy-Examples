...
fn extract_lightmaps(
  ...
  changed_lightmaps_query: Extract<
    Query<(Entity, &ViewVisibility, &Lightmap), Or<(Changed<ViewVisibility>, Changed<Lightmap>)>>,
  >,
  ...
  for (entity, view_visibility, lightmap) in changed_lightmaps_query.iter() {
    ...
    let (slab_index, slot_index) = render_lightmaps.allocate(&fallback_images, lightmap.image.id());
  ...
  render_lightmaps
    ...
    .retain(|&(slab_index, slot_index)| {
      let Some(asset_id) =
        render_lightmaps.slabs[usize::from(slab_index)].lightmaps[usize::from(slot_index)].asset_id
      else {
        error!("Allocated lightmap should have been removed from `pending_lightmaps` by now");
        return false;
      };
