...
fn compute_meshlets(
  ...
  let get_vertex = |&v: &u32| {
    *bytemuck::from_bytes::<Vec3>(
      &vertices.reader.get_ref()[vertices.position_offset + v as usize * vertices.vertex_stride..]
        [..12],
    )
  };
  ...
  for (group_id, group) in groups.iter().enumerate() {
    for &meshlet_id in group.meshlets.iter() {
      ...
      for index in meshlet.triangles {
        ...
        if position_only_locks[vertex_id] == -1 || position_only_locks[vertex_id] == group_id as i32
        {
  ...
  for i in 0..8 {
    ...
    if node.child_counts[i] == u8::MAX {
      ...
      for m in 0..node.child_counts[i] as u32 {
        ...
        let sphere_error = (Vec3A::from(sphere.center) - meshlet.lod_group_sphere.center).length()
