...
impl AssetInfos {
  ...
  pub(crate) fn get_or_create_path_handle_erased(
    ...
    let result =
      self.get_or_create_path_handle_internal(path, Some(type_id), loading_mode, meta_transform);
  ...
  fn process_handle_drop_internal(
    ...
    if watching_for_changes {
      Self::remove_dependents_and_labels(&info, loader_dependents, path, living_labeled_assets);
    }
