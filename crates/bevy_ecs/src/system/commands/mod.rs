...
const _: () = {
  ...
  unsafe impl bevy_ecs::system::SystemParam for Commands<'_, '_> {
    ...
    fn init_state(world: &mut World) -> Self::State {
      FetchState {
        state: <__StructFieldsAlias<'_, '_> as bevy_ecs::system::SystemParam>::init_state(world),
      }
    }
...
impl<'w, 's> Commands<'w, 's> {
  ...
  pub fn reborrow(&mut self) -> Commands<'w, '_> {
    Commands {
      queue: match &mut self.queue {
        ...
        InternalQueue::RawCommandQueue(queue) => InternalQueue::RawCommandQueue(queue.clone()),
      },
  ...
  pub fn spawn_empty(&mut self) -> EntityCommands<'_> {
    ...
    self
      .queue(move |world: &mut World| world.spawn_empty_at_with_caller(entity, caller).map(|_| ()));
    self.entity(entity)
  }
  ...
  /// It will internally return a [`RegisteredSystemError`](crate::system::system_registry::RegisteredSystemError),
  /// which will be handled by [logging the error at the `warn` level](warn).
  pub fn run_system_with<I>(&mut self, id: impl Into<SystemId<I>> + Send, input: I::Inner<'static>)
  where
