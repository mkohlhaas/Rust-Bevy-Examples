...
use bevy_ui::{
  px, AlignItems, AlignSelf, BoxShadow, Display, FlexDirection, GlobalZIndex, InteractionDisabled,
  JustifyContent, Node, OverrideClip, PositionType, Pressed, UiRect,
};
use bevy_ui_widgets::{
...
fn update_menuitem_styles_remove(
  ...
  removed_disabled
    ...
    .for_each(|ent| {
      if let Ok((item_ent, disabled, pressed, hovered, bg_color, font_color)) = q_menuitems.get(ent)
      {
        set_menuitem_colors(
          item_ent,
          disabled,
