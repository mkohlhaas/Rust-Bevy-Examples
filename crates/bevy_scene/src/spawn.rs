...
pub trait WorldSceneExt {
  ...
  fn spawn_scene_list<L: SceneList>(&mut self, scenes: L) -> Result<Vec<Entity>, SpawnSceneError>;
      ...
      .push((
        RelatedSceneListSpawn {
          ...
          insert: |entity, target| {
            entity.insert(<<T as RelationshipTarget>::Relationship as Relationship>::from(target));
          },
        },
        handle,
      ));
  ...
  for event in list_events.read() {
    match *event {
      ...
      AssetEvent::Removed { id } => {
        ...
        {
          let waiting_entities = waiting_related.iter().map(|r| r.entity).collect::<Vec<_>>();
          error!(
