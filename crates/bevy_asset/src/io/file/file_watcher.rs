        ...
        Ok(events) => {
          ...
          for event in events.iter() {
            ...
            let paths = event
              ...
              .iter()
              .map(PathBuf::as_path)
              .map(|p| make_absolute_path(p).expect("paths from the debouncer are valid"))
              .collect::<Vec<_>>();
            ...
            match event.kind {
              ...
              | notify::EventKind::Modify(ModifyKind::Name(RenameMode::From)) => {
                if let Some((path, is_meta)) = handler.get_path(&paths[0]) {
                  handler.handle(&paths, AssetSourceEvent::RemovedUnknown { path, is_meta });
                }
              }
              ...
              notify::EventKind::Modify(ModifyKind::Name(RenameMode::Both)) => {
                ...
                let Some((new_path, new_is_meta)) = handler.get_path(&paths[1]) else {
                  continue;
                };
