...
pub mod prelude {
  ...
  pub use crate::{
    bundle::Bundle,
    change_detection::{ContiguousMut, ContiguousRef, DetectChanges, DetectChangesMut, Mut, Ref},
    children,
    component::Component,
    ...
    resource::Resource,
    schedule::{
      common_conditions::*, ApplyDeferred, IntoScheduleConfigs, IntoSystemSet, Schedule, Schedules,
      SystemCondition, SystemSet,
    },
    spawn::{Spawn, SpawnIter, SpawnRelated, SpawnWith, WithOneRelated, WithRelated},
    system::{
      Command, Commands, Deferred, EntityCommand, EntityCommands, If, In, InMut, InRef, IntoSystem,
      Local, NonSend, NonSendMut, ParamSet, Populated, Query, ReadOnlySystem, Res, ResMut, Single,
      System, SystemIn, SystemInput, SystemParamBuilder, SystemParamFunction,
    },
    template::{template, FromTemplate, Template},
    world::{
      EntityMut, EntityRef, EntityWorldMut, FilteredResources, FilteredResourcesMut, FromWorld,
      World,
    },
  };
...
mod tests {
  ...
  fn bundle_derive() {
    ...
    let ids: Vec<_> =
      <BundleWithIgnored as Bundle>::component_ids(&mut world.components_registrator()).collect();
