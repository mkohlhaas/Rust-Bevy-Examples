...
impl DeriveComponent {
  ...
  pub fn parse(ast: &DeriveInput, storage_attr: StorageAttribute) -> Result<DeriveComponent> {
    ...
    for attr in ast.attrs.iter() {
      if attr.path().is_ident(COMPONENT) {
        ...
        let punctuated = attr.parse_args_with(Punctuated::<Require, Comma>::parse_terminated)?;
  ...
  pub fn impl_component(
    ...
    let on_remove_path = Vec::from_iter(self.on_remove.map(|path| path.to_token_stream(bevy_ecs)));
    ...
    if relationship.is_some() {
      ...
      on_discard_path.push(quote!(<Self as #bevy_ecs::relationship::Relationship>::on_discard));
    }
    if let Some(target) = self.relationship_target {
      ...
      if target.linked_spawn {
        on_despawn_path
          .push(quote!(<Self as #bevy_ecs::relationship::RelationshipTarget>::on_despawn));
      }
    }
    ...
    let on_discard = hook_register_function_call(bevy_ecs, quote! {on_discard}, &on_discard_path);
    let on_remove = hook_register_function_call(bevy_ecs, quote! {on_remove}, &on_remove_path);
    let on_despawn = hook_register_function_call(bevy_ecs, quote! {on_despawn}, &on_despawn_path);
  ...
  fn derive_relationship(&self, ast: &DeriveInput, bevy_ecs: &Path) -> Result<Option<TokenStream>> {
  ...
  fn parse(input: syn::parse::ParseStream) -> Result<Self> {
    ...
    Ok(RelationshipTarget {
      relationship: relationship
        .ok_or_else(|| syn::Error::new(input.span(), "Missing `relationship = X` attribute"))?,
      linked_spawn,
    })
  }
}
