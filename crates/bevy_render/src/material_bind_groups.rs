...
use crate::{
  render_resource::{
    ...
    BindlessResourceType, Buffer, BufferBinding, BufferDescriptor, BufferId, BufferInitDescriptor,
    BufferUsages, CompareFunction, FilterMode, MipmapFilterMode, OwnedBindingResource,
...
impl<'a> From<&'a OwnedBindingResource> for BindingResourceId {
  fn from(value: &'a OwnedBindingResource) -> Self {
    match *value {
      ...
      OwnedBindingResource::Sampler(_, ref sampler) => BindingResourceId::Sampler(sampler.id()),
    }
  }
}
...
impl MaterialBindGroupAllocator {
  ...
  pub fn new(
    ...
    if let Some(bindless_descriptor) = bindless_descriptor {
      ...
      MaterialBindGroupAllocator::NonBindless(Box::new(MaterialBindGroupNonBindlessAllocator::new(
        label,
      )))
    }
  }
  ...
  pub fn get(&self, group: MaterialBindGroupIndex) -> Option<MaterialSlab<'_>> {
    match *self {
      ...
      MaterialBindGroupAllocator::NonBindless(ref non_bindless_allocator) => non_bindless_allocator
        .get(group)
        .map(|non_bindless_slab| MaterialSlab(MaterialSlabImpl::NonBindless(non_bindless_slab))),
    }
  }
  ...
  pub fn allocate_unprepared(
    ...
    match *self {
      MaterialBindGroupAllocator::Bindless(ref mut material_bind_group_bindless_allocator) => {
...
impl MaterialBindlessIndexTable {
  ...
  fn new(
    ...
    for _ in
      *bindless_index_table_descriptor.indices.start..*bindless_index_table_descriptor.indices.end
    {
      buffer.push(0);
    }
  ...
  fn get_binding(&self, slot: MaterialBindGroupSlot, bindless_index: BindlessIndex) -> Option<u32> {
...
impl MaterialBindlessSlab {
  ...
  fn try_allocate(
    ...
    if self.allocated_resource_count > 0
      && self.allocated_resource_count + allocation_candidate.needed_free_slots > slot_capacity
    {
  ...
  fn free(&mut self, slot: MaterialBindGroupSlot, bindless_descriptor: &BindlessDescriptor) {
    ...
    for (bindless_index, bindless_resource_type) in bindless_descriptor.resources.iter().enumerate()
    {
      ...
      let Some(bindless_binding) = bindless_index_table.get_binding(slot, bindless_index) else {
        continue;
      };
        ...
        BindlessResourceType::DataBuffer => {
          ...
          data_buffers.insert(
            ...
            MaterialDataBuffer::new(
              ...
              buffer_descriptor
                .size
                .expect("Data buffers should have a size") as u32,
            ),
          );
        }
        BindlessResourceType::SamplerFiltering
...
impl MaterialBindGroupNonBindlessAllocator {
  ...
  fn prepare_bind_groups(&mut self, render_device: &RenderDevice, pipeline_cache: &PipelineCache) {
...
impl<'a> MaterialSlab<'a> {
  ...
  pub fn bind_group(&self) -> Option<&'a BindGroup> {
    match self.0 {
      MaterialSlabImpl::Bindless(material_bindless_slab) => material_bindless_slab.bind_group(),
      MaterialSlabImpl::NonBindless(MaterialNonBindlessSlab::Prepared(prepared_bind_group)) => {
      ...
      Ok(unprepared) => {
        ...
        match self.entry(material_id.into()) {
          Entry::Occupied(mut occupied_entry) => {
            ...
            let new_binding = bind_group_allocator.allocate_unprepared(unprepared, material_layout);
            *occupied_entry.get_mut() = new_binding;
            Ok(new_binding)
          }
          Entry::Vacant(vacant_entry) => Ok(
            *vacant_entry
      ...
      Err(AsBindGroupError::CreateBindGroupDirectly) => {
        match material.as_bind_group(
          ...
          Ok(prepared_bind_group) => {
            ...
            let material_binding_id = bind_group_allocator.allocate_prepared(prepared_bind_group);
