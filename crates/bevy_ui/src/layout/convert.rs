...
impl Val {
  fn into_length_percentage_auto(
    ...
    match self {
      ...
      Val::VMin(value) => style_helpers::length(context.physical_size.min_element() * value / 100.),
      Val::VMax(value) => style_helpers::length(context.physical_size.max_element() * value / 100.),
...
impl MinTrackSizingFunction {
  fn into_taffy(self, context: &LayoutContext) -> taffy::style::MinTrackSizingFunction {
    match self {
      ...
      MinTrackSizingFunction::MinContent => taffy::style::MinTrackSizingFunction::min_content(),
      MinTrackSizingFunction::MaxContent => taffy::style::MinTrackSizingFunction::max_content(),
      MinTrackSizingFunction::VMin(val) => Val::VMin(val).into_length_percentage(context).into(),
      MinTrackSizingFunction::VMax(val) => Val::VMax(val).into_length_percentage(context).into(),
...
impl MaxTrackSizingFunction {
  fn into_taffy(self, context: &LayoutContext) -> taffy::style::MaxTrackSizingFunction {
    match self {
      ...
      MaxTrackSizingFunction::MinContent => taffy::style::MaxTrackSizingFunction::min_content(),
      MaxTrackSizingFunction::MaxContent => taffy::style::MaxTrackSizingFunction::max_content(),
      ...
      MaxTrackSizingFunction::VMin(val) => Val::VMin(val).into_length_percentage(context).into(),
      MaxTrackSizingFunction::VMax(val) => Val::VMax(val).into_length_percentage(context).into(),
