...
use bevy_render::{
  ...
  render_resource::{
    ...
    BindGroup, BindGroupEntry, BindGroupLayoutEntries, Buffer, BufferBindingType, BufferDescriptor,
    BufferInitDescriptor, BufferUsages, ColorTargetState, ColorWrites, CommandEncoder,
    ComputePassDescriptor, ComputePipeline, Extent3d, IndexFormat, LoadOp, MapMode, Operations,
    PipelineCache, RenderPassColorAttachment, RenderPassDescriptor, RenderPipeline, ShaderStages,
    ShaderType, SpecializedComputePipeline, SpecializedComputePipelines, SpecializedRenderPipeline,
    SpecializedRenderPipelines, StorageBuffer, StoreOp, TextureDescriptor, TextureDimension,
    TextureFormat, TextureUsages, UninitBufferVec, VertexAttribute, VertexStepMode,
  },
...
impl ViewClusteringReadbackData {
  ...
  fn update_from_metadata(&mut self, gpu_clustering_metadata: &ClusterMetadata) {
    ...
    self.last_frame_statistics = Some(ViewClusteringLastFrameStatistics {
      ...
      farthest_z: f32::from_bits(sortable_u32_to_f32_bits(gpu_clustering_metadata.farthest_z)),
    });
  }
}
...
impl FromWorld for ClusteringRasterPipeline {
  fn from_world(world: &mut World) -> Self {
    ...
    let mut bind_group_layout_entries_count_pass = vec![
      ...
      binding_types::uniform_buffer::<GpuLights>(true).build(5, ShaderStages::VERTEX_FRAGMENT),
      // @group(0) @binding(6) var<uniform> view: View;
      binding_types::uniform_buffer::<ViewUniform>(true).build(6, ShaderStages::VERTEX_FRAGMENT),
    ];

    let mut bind_group_layout_entries_populate_pass = bind_group_layout_entries_count_pass.clone();
...
fn cluster_on_gpu(
  ...
  let (
    ...
    pipeline_cache
      .get_compute_pipeline(view_gpu_clustering_pipeline_ids.clustering_z_slicing_pipeline_id),
    pipeline_cache
    ...
    command_encoder.map_buffer_on_submit(staging_buffer, MapMode::Read, .., move |result| {
...

      {
        ...
        let buffer_view = captured_staging_buffer
          ...
          .get_mapped_range()
          .unwrap();
        let Ok(mut buffer_reader) = Reader::new::<ClusterMetadata>(buffer_view[..].to_vec(), 0)
        else {
          return;
        };
...
fn prepare_clustering_bind_groups(
  ...
  for (view_entity, view_gpu_clustering_buffers, view_cluster_bindings) in &views_query {
    ...
    let clustering_bind_group_allocate_pass = render_device.create_bind_group(
      ...
      &pipeline_cache.get_bind_group_layout(&clustering_allocation_pipeline.bind_group_layout),
...
pub fn extract_clusters_for_gpu_clustering(
    ...
    views.iter_mut(&mut main_world)
  {
      ...
      .get(&MainEntity::from(main_view_entity))
    {
      ...
      if let Some(last_frame_statistics) = &view_clustering_buffer_size_data.last_frame_statistics {
