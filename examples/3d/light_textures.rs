    ...
    for (mut transform, spotlight_option, base_scale, selection) in &mut selections {
      ...
      match app_number_input {
        ...
        AppNumberInput::Roll => {
          ...
          transform.rotation = Quat::from_euler(EulerRot::YXZ, yaw, pitch, value_change.value);
        }
      }
    }
    commands
...
fn handle_drag_as_movement(
  ...
  for (mut transform, selection) in &mut selections {
    ...
    if *selection == Selection::PointLight {
      transform.translation += (mouse_motion.delta * Vec2::new(1.0, -1.0) * MOVE_SPEED).extend(0.0);
      return;
    }
    ...
    transform.translation = radius * vec3(sin(theta) * cos(phi), cos(theta), sin(theta) * sin(phi));
