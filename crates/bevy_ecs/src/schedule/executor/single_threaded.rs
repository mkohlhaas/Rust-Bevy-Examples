  ...
  conditions
    ...
    .map(|ConditionWithAccess { condition, .. }| {
      ...
      let f = |condition: &mut BoxedCondition| {
        __rust_begin_short_backtrace::readonly_run(&mut **condition, world).unwrap_or_else(|err| {
      ...
      #[cfg(feature = "std")]
      let result = handle_unwind_in_run_condition(f, condition, for_system, on_set, error_handler);
      result
    })
...
fn handle_unwind(
  ...
  if let Err(payload) = potential_unwind {
    ...
    let err = BevyError::new_with_backtrace(Severity::Panic, error_message, Backtrace::disabled());
...
fn handle_unwind_in_run_condition(
  ...
  match potential_unwind {
    ...
    Err(payload) => {
      ...
      let err =
        BevyError::new_with_backtrace(Severity::Panic, "Encountered panic", Backtrace::disabled());
      __rust_begin_short_backtrace::error_handler(
        error_handler,
        err,
        ErrorContext::RunCondition {
