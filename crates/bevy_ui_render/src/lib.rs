...
pub fn extract_viewport_nodes(
  ...
  for (entity, uinode, stack_index, transform, inherited_visibility, clip, camera, viewport_node) in
    extracted_uinodes
      .changed
      .iter()
      .flat_map(|main_entity| uinode_query.get(main_entity.entity()).ok())
    ...
    .flat_map(|main_entity| uinode_query.get(main_entity.entity()).ok())
  {
    ...
    let transform = Affine2::from(*global_transform)
      * Affine2::from_translation(
        uinode.content_box().min - editable_text.map_or(Vec2::ZERO, |text| text.viewport.offset),
      );
    ...
    .flat_map(|main_entity| uinode_query.get(main_entity.entity()).ok())
  {
    ...
    ) in text_layout_info.glyphs.iter().enumerate()
    {
      ...
      if text_layout_info.glyphs.get(i + 1).is_none_or(|info| {
        info.section_index != *section_index || info.atlas_info.texture != atlas_info.texture
      }) {
        extracted_uinodes
          .uinodes
          .entry(entity.into())
    ...
    for run in text_layout_info.run_geometry.iter() {
      ...
      let Ok((has_strikethrough, has_underline)) = text_decoration_query.get(section_entity) else {
        continue;
      };
