use crate::{
  ...
  render_resource::{
    BindGroup, BindGroupEntries, Buffer, BufferUsages, PipelineCache, SpecializedRenderPipeline,
    SpecializedRenderPipelines, Texture, TextureUsages, TextureView,
  },
        ...
        Ok(format) => {
          ...
          #[cfg(target_arch = "wasm32")]
          {
            let save_screenshot = || {
              ...
              img
                .write_to(&mut image_buffer, format)
                .map_err(|e| JsValue::from_str(&format!("{e}")))?;

              let parts = js_sys::Array::of1(
                &js_sys::Uint8Array::new_from_slice(image_buffer.into_inner().as_bytes()).into(),
              );
...
fn extract_screenshots(
  ...
  let (mut commands, primary_window, screenshots) = system_state.get_mut(&mut main_world).unwrap();
  ...
  for (entity, target) in targets.iter() {
    match target {
      NormalizedRenderTarget::Window(window) => {
        ...
        let Some((_, surface_data)) = window_surfaces.iter().find(|(e, _)| *e == window) else {
