...
fn drag_by_header(
  ...
  if let Ok(child_of) = diagnostics_overlay_headers.get(entity) {
    ...
    let Ok(mut node) = diagnostics_overlays.get_mut(child_of.get()) else {
      panic!(
        "DiagnosticsOverlayHeader has been tempered with. Parent was not a DiagnosticsOverlay."
      );
    };
    let delta = event.delta;
    let Val::Px(top) = &mut node.top else {
      panic!("DiagnosticsOverlay has been tempered with. Node must have `top` using `Val::Px`.");
    };
    *top += delta.y;
    let Val::Px(left) = &mut node.left else {
      panic!("DiagnosticsOverlay has been tempered with. Node must have `left` using `Val::Px`.");
    };
    *left += delta.x;
  }
}
