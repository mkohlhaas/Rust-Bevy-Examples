...
impl<'w, 's, R: TextSection> TextReader<'w, 's, R> {
  ...
  pub fn get_letter_spacing(&mut self, root_entity: Entity, index: usize) -> Option<LetterSpacing> {
    self
      .get(root_entity, index)
...
impl<'w, 's, R: TextSection> TextWriter<'w, 's, R> {
  ...
  pub fn for_each_color(&mut self, root_entity: Entity, mut callback: impl FnMut(Mut<TextColor>)) {
    ...
    loop {
      ...
      loop {
        ...
        *idx += 1;
        ...
        let entity = *child;
        let Ok((text, font, color, line_height, letter_spacing)) = self.spans.get_mut(entity)
        else {
          continue;
        };
