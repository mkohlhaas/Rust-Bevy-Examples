...
impl<T: EntityEquivalent> UniqueEntityEquivalentSlice<T> {
  ...
  pub fn chunks(&self, chunk_size: usize) -> Chunks<'_, T> {
    // SAFETY: Any subslice of a unique slice is also unique.
    unsafe { UniqueEntityEquivalentSliceIter::from_slice_iter_unchecked(self.0.chunks(chunk_size)) }
  }
  ...
  pub fn chunks_exact(&self, chunk_size: usize) -> ChunksExact<'_, T> {
    // SAFETY: Any subslice of a unique slice is also unique.
    unsafe {
      UniqueEntityEquivalentSliceIter::from_slice_iter_unchecked(self.0.chunks_exact(chunk_size))
    }
  }
  ...
  pub fn rchunks_exact(&self, chunk_size: usize) -> RChunksExact<'_, T> {
    // SAFETY: Any subslice of a unique slice is also unique.
    unsafe {
      UniqueEntityEquivalentSliceIter::from_slice_iter_unchecked(self.0.rchunks_exact(chunk_size))
    }
  }
    ...
    F: FnMut(&T, &T) -> bool,
  {
    // SAFETY: Any subslice of a unique slice is also unique.
    unsafe {
      UniqueEntityEquivalentSliceIterMut::from_mut_slice_iter_unchecked(self.0.chunk_by_mut(pred))
    }
  }
    ...
    F: FnMut(&T) -> bool,
  {
    // SAFETY: Any subslice of a unique slice is also unique.
    unsafe {
      UniqueEntityEquivalentSliceIterMut::from_mut_slice_iter_unchecked(self.0.split_mut(pred))
    }
  }
    ...
    F: FnMut(&T) -> bool,
  {
    // SAFETY: Any subslice of a unique slice is also unique.
    unsafe {
      UniqueEntityEquivalentSliceIterMut::from_mut_slice_iter_unchecked(self.0.rsplit_mut(pred))
    }
  }
  ...
  {
    // SAFETY: Any subslice of a unique slice is also unique.
    unsafe { UniqueEntityEquivalentSliceIter::from_slice_iter_unchecked(self.0.splitn(n, pred)) }
  }
    ...
    F: FnMut(&T) -> bool,
  {
    // SAFETY: Any subslice of a unique slice is also unique.
    unsafe {
      UniqueEntityEquivalentSliceIterMut::from_mut_slice_iter_unchecked(self.0.splitn_mut(n, pred))
    }
  }
  ...
  {
    // SAFETY: Any subslice of a unique slice is also unique.
    unsafe { UniqueEntityEquivalentSliceIter::from_slice_iter_unchecked(self.0.rsplitn(n, pred)) }
  }
    ...
    F: FnMut(&T) -> bool,
  {
    // SAFETY: Any subslice of a unique slice is also unique.
    unsafe {
      UniqueEntityEquivalentSliceIterMut::from_mut_slice_iter_unchecked(self.0.rsplitn_mut(n, pred))
    }
  }
  ...
  for Box<UniqueEntityEquivalentSlice<T>>
{
  fn from(value: UniqueEntityEquivalentArray<T, N>) -> Self {
    // SAFETY: All elements in the original slice are unique.
    unsafe { UniqueEntityEquivalentSlice::from_boxed_slice_unchecked(Box::new(value.into_inner())) }
  }
}
...
impl<'a, T: EntityEquivalent> UniqueEntityIter<slice::IterMut<'a, T>> {
  ...
  pub fn into_slice(self) -> &'a mut UniqueEntityEquivalentSlice<T> {
    // SAFETY: All elements in the original slice are unique.
    unsafe { UniqueEntityEquivalentSlice::from_slice_unchecked_mut(self.into_inner().into_slice()) }
  }
...
impl<'a, T: EntityEquivalent + 'a, I: Iterator<Item = &'a mut [T]> + AsMut<[&'a mut [T]]>>
  AsMut<[&'a mut UniqueEntityEquivalentSlice<T>]> for UniqueEntityEquivalentSliceIterMut<'a, T, I>
{
