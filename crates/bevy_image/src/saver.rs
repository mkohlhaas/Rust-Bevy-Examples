  ...
  async fn save(
    ...
    let (image_crate_format, color_type, is_srgb): (_, ExtendedColorType, _) = match format {
      ...
      ImageFormat::Png => match asset.texture_descriptor.format {
        ...
        TextureFormat::Rgba8Unorm => (image::ImageFormat::Png, ExtendedColorType::Rgba8, false),
        TextureFormat::Rgba8UnormSrgb => (image::ImageFormat::Png, ExtendedColorType::Rgba8, true),
        _ => {
...
pub enum SaveImageError {
  ...
  #[error(
    "SaveImageFormatSetting::FromExtension was set, but the asset path \"{0}\" has no extension"
  )]
