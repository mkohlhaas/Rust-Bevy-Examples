...
impl Plugin for RemotePlugin {
  fn build(&self, app: &mut App) {
      ...
      remote_methods.insert(
        ...
        match handler {
          RemoteMethodHandler::Instant(system) => {
            RemoteMethodSystemId::Instant(app.main_mut().world_mut().register_boxed_system(system))
          }
          RemoteMethodHandler::Watching(system) => {
            RemoteMethodSystemId::Watching(app.main_mut().world_mut().register_boxed_system(system))
          }
        },
      );
    }
    ...
    {
      app
        .init_resource::<PreviousScheduleBuildMetadata>()
    ...
    #[cfg(feature = "bevy_render")]
    {
      ...
      for (name, handler) in render_plugin_methods.drain(..) {
        render_remote_methods.insert(
          ...
          match handler {
            RemoteMethodHandler::Instant(system) => {
              RemoteMethodSystemId::Instant(render_app.world_mut().register_boxed_system(system))
            }
            RemoteMethodHandler::Watching(system) => {
              RemoteMethodSystemId::Watching(render_app.world_mut().register_boxed_system(system))
            }
          },
        );
      }
