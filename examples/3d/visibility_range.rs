...
fn setup(
  ...
  commands.spawn((
    WorldAssetRoot(asset_server.load(
      GltfAssetLabel::Scene(0).from_asset("models/FlightHelmetLowPoly/FlightHelmetLowPoly.gltf"),
    )),
    MainModel::LowPoly,
  ));
...
fn update_mode(
  ...
  for (mut visibility_range, main_model) in meshes.iter_mut() {
    *visibility_range = match (main_model, app_status.show_one_model_only) {
      ...
      | (&MainModel::LowPoly, Some(MainModel::HighPoly)) => INVISIBLE_VISIBILITY_RANGE.clone(),
      (&MainModel::HighPoly, Some(MainModel::HighPoly))
      | (&MainModel::LowPoly, Some(MainModel::LowPoly)) => SINGLE_MODEL_VISIBILITY_RANGE.clone(),
