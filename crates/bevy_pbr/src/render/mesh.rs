...
use bevy_light::{
  EnvironmentMapLight, IrradianceVolume, NotShadowCaster, NotShadowReceiver, ShadowFilteringMethod,
  TransmittedShadowReceiver,
};
use bevy_math::{Affine3, Affine3Ext, Rect, UVec2, Vec3, Vec4};
use bevy_mesh::{
  skinning::SkinnedMesh, BaseMeshPipelineKey, Mesh, Mesh3d, MeshAttributeCompressionFlags, MeshTag,
  MeshVertexBufferLayoutRef, VertexAttributeDescriptor,
};
...
use crate::{
  render::{
    morph::{
      extract_morphs, no_automatic_morph_batching, write_morph_buffers, MorphIndices, MorphUniforms,
    },
...
use bevy_render::view::{
  texture_format_from_code, texture_format_to_code, ExtractedView, RenderShadowMapVisibleEntities,
...
impl RenderMeshInstances {
  ...
  pub fn render_mesh_queue_data(&self, entity: MainEntity) -> Option<RenderMeshQueueData<'_>> {
    match *self {
      RenderMeshInstances::CpuBuilding(ref instances) => instances.render_mesh_queue_data(entity),
      RenderMeshInstances::GpuBuilding(ref instances) => instances.render_mesh_queue_data(entity),
    }
  }
...
impl MeshCullingData {
  ...
  fn update(&self, mesh_culling_data_buffer: &mut MeshCullingDataBuffer, instance_data_index: u32) {
...
pub fn extract_meshes_for_gpu_building(
  ...
  for entity in removed_meshes_query.read() {
    ...
    if !changed_meshes_query.contains(*entity) && !meshes_to_reextract_next_frame.contains(&entity)
    {
...
fn collect_gpu_culled_meshes_for_subview(
  ...
  for main_entity in &render_mesh_instance_gpu_queues.changed_layers {
    let Some(new_render_layers) = render_mesh_instance_gpu_queues.entities.get(main_entity) else {
      continue;
    };
...
pub fn collect_meshes_for_gpu_building(
  ...
  let RenderMeshInstances::GpuBuilding(render_mesh_instances) = render_mesh_instances.into_inner()
  else {
    return;
  };
        ...
        match *queue {
          ...
          RenderMeshInstanceGpuQueue::GpuCulling {
            ...
            scope.spawn(async move {
              #[cfg(feature = "trace")]
              let _span = info_span!("prepared_mesh_producer").entered();
              for (entity, mesh_instance_builder, mesh_culling_builder) in changed_cpu_culling
                .drain(..)
                .chain(changed_gpu_culling.drain(..))
              {
                ...
                let Some(render_mesh_instance) = render_mesh_instances.get(&entity) else {
                  ...
                  let _ = prepared_tx.send((entity, prepared, Some(mesh_culling_builder)));
                  continue;
                };
                ...
                prepared.mesh_input_uniform.previous_input_index = previous_input_index;

                current_input_buffer.set(current_uniform_index, prepared.mesh_input_uniform);
                mesh_culling_data_buffer.set(current_uniform_index, mesh_culling_builder);
              }
...
pub fn build_dummy_white_gpu_image(
  ...
  let sampler = match image.sampler {
    ...
    ImageSampler::Descriptor(ref descriptor) => render_device.create_sampler(&descriptor.as_wgpu()),
  };
...
impl MeshPipelineKey {
  ...
  const VIEW_PROJECTION_SHIFT_BITS: u64 = Self::SHADOW_FILTER_METHOD_MASK_BITS.count_ones() as u64
  ...
  pub fn strip_index_format(&self) -> Option<IndexFormat> {
    ...
    match index_bits {
      x if x == BaseMeshPipelineKey::STRIP_INDEX_FORMAT_U16.bits() => Some(IndexFormat::Uint16),
      x if x == BaseMeshPipelineKey::STRIP_INDEX_FORMAT_U32.bits() => Some(IndexFormat::Uint32),
...
fn is_skinned(layout: &MeshVertexBufferLayoutRef) -> bool {
  layout.0.contains(Mesh::ATTRIBUTE_JOINT_INDEX) && layout.0.contains(Mesh::ATTRIBUTE_JOINT_WEIGHT)
}
...
impl MeshPhaseBindGroups {
  ...
  pub fn get(
    ...
    match (is_skinned, morph, lightmap) {
      ...
      (_, MeshMorphBindGroupKey::Storage(metadata_and_morph_slab_ids), _) => {
        match &self.morph_targets {
...
pub fn prepare_mesh_bind_groups(
      ...
      .instance_data_binding()
  {
    ...
    let mut cpu_preprocessing_mesh_bind_groups = match mesh_bind_groups.as_deref_mut() {
      None | Some(MeshBindGroups::GpuPreprocessing(_)) => MeshPhaseBindGroups::new(&render_device),
  ...
  if let Some(gpu_batched_instance_buffers) = gpu_batched_instance_buffers {
    ...
    {
      let Some(instance_data_binding) = batched_phase_instance_buffers.instance_data_binding()
      else {
        continue;
      };
  ...
  for metadata_slab_id in mesh_allocator.metadata_slabs() {
    ...
    if weights_uniform.current_buffer.buffer().is_some() {
      match (render_morph_target_allocator, &mut groups.morph_targets) {
        ...
        _ => {
          error!("Mismatched render morph target allocator and mesh morph target bind groups");
        }
      }
    }
...
impl<P: PhaseItem, const I: usize> RenderCommand<P> for SetMeshBindGroup<I> {
  ...
  fn render<'w>(
    ...
    let Some(mesh_phase_bind_groups) = (match *bind_groups {
      MeshBindGroups::CpuPreprocessing(ref mesh_phase_bind_groups) => Some(mesh_phase_bind_groups),
