...
pub mod prelude {
  ...
  #[doc(hidden)]
  pub use crate::{
    asset_value, Asset, AssetApp, AssetEvent, AssetId, AssetMode, AssetPlugin, AssetServer, Assets,
  ...
  fn build(&self, app: &mut App) {
    let embedded = EmbeddedAssetRegistry::default();
    {
      ...
      sources.init_default_source(
        &self.file_path,
        (!matches!(self.mode, AssetMode::Unprocessed)).then_some(self.processed_file_path.as_str()),
      );
      ...
      match self.mode {
        ...
        AssetMode::Processed => {
          ...
          if use_asset_processor {
            ...
            app
              .insert_resource(AssetServer::new_with_loaders(
                sources,
...
impl AssetApp for App {
    ...
    A: Asset + Reflect + FromReflect + GetTypeRegistration,
  {
    let type_registry = self.world().resource::<AppTypeRegistry>();
    {
      ...
      type_registry.register_type_conversion::<String, HandleTemplate<A>, _>(|s| Ok(s.into()));
    }
    ...
    run_app_until(&mut app, |world| {
      ...
      for event in cursor.read(events) {
          ...
          && *id == handle.id()
        {
          ...
          let a_handle: Handle<CoolText> = asset_server.get_handle("text/a.cool.ron").unwrap();
          let c_handle: Handle<CoolText> = asset_server.get_handle("text/c.cool.ron").unwrap();
          ...
          AssetLoadError::AssetReaderError(read_error) => match read_error {
            AssetReaderError::Io(_) => {
              ...
              if tracker.failures <= 2 {
                ...
                tracker
                  .queued_retries
                  .push((error.path.clone(), error.id, now + 10));
              } else {
                panic!("Unexpected failure #{} (expected only 2)", tracker.failures);
              }
            }
...
(
  ...
  fn collect_asset_load_failed_events<A: Asset>(world: &mut World) -> Vec<AssetLoadFailedEvent<A>> {
    world
...
(
  ...
  fn resource_are_dependencies_loaded() {
    ...
    assert!(!asset_server.are_direct_dependencies_loaded(app.world().resource::<MyAssetHolder>()));
