...
use crate::{
  ...
  converters::{
    convert_enabled_buttons, convert_resize_direction, convert_window_level, convert_window_theme,
    convert_winit_theme,
  },
...
pub(crate) fn changed_cursor_options(
  ...
  WINIT_WINDOWS.with_borrow(|winit_windows| {
    for (entity, window, mut cursor_options, mut cache) in &mut changed_windows {
      ...
      if let Err(err) = crate::winit_windows::attempt_grab(winit_window, cursor_options.grab_mode) {
        warn!(
