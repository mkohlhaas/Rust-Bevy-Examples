...
use bevy_render::{
  ...
  render_resource::{
    ...
    FilterMode, MipmapFilterMode, PipelineCache, Sampler, SamplerBindingType, SamplerDescriptor,
    ShaderStages, ShaderType, SpecializedComputePipelines, StorageTextureAccess, TextureAspect,
  ...
  fn build(&self, app: &mut App) {
    ...
    let general_shaders: HashMap<_, _> = TEXTURE_FORMATS
      ...
      .map(|(target_format, identifier)| {
        let shader_source = shader_template_source.replace("##TEXTURE_FORMAT##", identifier);
        (
          *target_format,
...
pub fn generate_mips_for_phase(
  ...
  for mip_generation_job in mip_generation_phase.iter() {
    ...
    let (Some(mip_generation_pipeline_pass_1), Some(mip_generation_pipeline_pass_2)) = (
      pipeline_cache.get_compute_pipeline(mip_generation_pipelines.downsampling_pipeline_pass_1),
      pipeline_cache.get_compute_pipeline(mip_generation_pipelines.downsampling_pipeline_pass_2),
    ) else {
      continue;
    };
  ...
  match mip_generation_pipelines.entry(target_format) {
    Entry::Vacant(vacant_entry) => {
      ...
      let (downsampling_bind_group_layout_pass_1, downsampling_bind_group_layout_pass_2) =
        create_downsampling_bind_group_layouts(target_format, combine_downsampling_bind_groups);
