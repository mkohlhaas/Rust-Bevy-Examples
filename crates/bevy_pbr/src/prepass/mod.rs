...
use crate::{
  ...
  RenderMeshInstanceFlags, RenderMeshInstances, SetMaterialBindGroup, SetMeshBindGroup, ShadowView,
};
...
use bevy_render::{
  ...
  Extract, ExtractSchedule, GpuResourceAppExt, Render, RenderApp, RenderDebugFlags, RenderStartup,
  RenderSystems,
};
  ...
  fn specialize(
    ...
    if mesh_key.intersects(MeshPipelineKey::NORMAL_PREPASS | MeshPipelineKey::DEFERRED_PREPASS) {
      ...
      if layout.0.contains(Mesh::ATTRIBUTE_NORMAL) {
        ...
        warn!("The default normal prepass expects the mesh to have vertex normal attributes.");
      }
...
pub(crate) struct SpecializePrepassSystemParam<'w, 's> {
  ...
  specialized_prepass_material_pipeline_cache: ResMut<'w, SpecializedPrepassMaterialPipelineCache>,
...

  {
    ...
    for (extracted_view, visible_entities, msaa, motion_vector_prepass, deferred_prepass) in &views
    {
      ...
      let mut maybe_specialized_prepass_material_pipeline_cache =
        specialized_prepass_material_pipeline_cache.get_mut(&extracted_view.retained_view_entity);
...
pub fn queue_prepass_material_meshes(
  ...
  for (extracted_view, visible_entities) in &views {
    ...
    for (render_entity, visible_entity) in dirty_specializations.iter_to_queue(
      ...
      // materials and bail.
      let Some(material_instance) = render_material_instances.instances.get(visible_entity) else {
        view_pending_prepass_mesh_material_queues
          .current_frame
          .insert((*render_entity, *visible_entity));
        continue;
