...
pub fn load_assets(
  ...
  let car_texture: Handle<Image> = load_asset!(format!("{base_url}/car-kit/Textures/colormap.png"));
  ...
  let cars = {
    ...
    .iter()
    .map(
      |t| load_asset!(GltfAssetLabel::Scene(0).from_asset(format!("{base_url}/car-kit/{t}.glb"))),
    )
    .collect::<Vec<_>>()
  };
  ...
  let tree_small: Handle<WorldAsset> = load_asset!(
    GltfAssetLabel::Scene(0).from_asset(format!("{base_url}/city-kit-suburban/tree-small.glb"))
  );
  let tree_large: Handle<WorldAsset> = load_asset!(
  ...
  let tree_lod_material = materials.add(StandardMaterial::from_color(Color::srgb_u8(90, 196, 135)));
  let tree_small_lod = {
