...
impl World {
  ...
  pub fn inspect_entity(
    ...
    let archetype = self.archetypes().get(entity_location.archetype_id).expect(
      "ArchetypeId was retrieved from an EntityLocation and should correspond to an Archetype",
    );
  ...
  pub fn modify_component<T: Component, R>(
    ...
    let result =
      world.modify_component_with_relationship_hook_mode(entity, RelationshipHookMode::Run, f)?;
  ...
  pub fn init_resource<R: Resource + FromWorld>(&mut self) -> ComponentId {
    ...
    self
      .insert_resource_if_not_exists_with_caller(R::from_world, caller)
      .0
  }
      ...
      match self.entities().get_spawned(first_entity) {
        ...
        Ok(first_location) => {
          let mut cache = InserterArchetypeCache {
            ...
            inserter: unsafe {
              BundleInserter::new_with_id(self, first_location.archetype_id, bundle_id, change_tick)
            },
        ...
        if let Ok(location) = cache.inserter.entities().get_spawned(entity) {
          if location.archetype_id != cache.archetype_id {
            cache = InserterArchetypeCache {
              // SAFETY: we initialized this bundle_id in `register_info`
              inserter: unsafe {
                BundleInserter::new_with_id(self, location.archetype_id, bundle_id, change_tick)
              },
              archetype_id: location.archetype_id,
            }
          }
  ...
  pub fn iter_resources_mut(&mut self) -> impl Iterator<Item = (&ComponentInfo, MutUntyped<'_>)> {
    ...
    resource_entities
      ...
      .filter_map(move |(component_id, entity)| {
        ...
        let component_info = unsafe { components.get_info(component_id).debug_checked_unwrap() };
