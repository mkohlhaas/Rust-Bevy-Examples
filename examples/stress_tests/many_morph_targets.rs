...
impl FromStr for ArgSpawning {
  ...
  fn from_str(s: &str) -> Result<Self, Self::Err> {
    match s {
      ...
      _ => Err(
        "must be 'instant', 'gradual', 'regular-cycle', 'random-cycle', or 'random-steady'".into(),
      ),
    }
  }
}
...
fn update(
  ...
  for mesh_index in to_spawn {
    ...
    let animation_asset = assets.animations[mesh_index.rem_euclid(assets.animations.len())].clone();
