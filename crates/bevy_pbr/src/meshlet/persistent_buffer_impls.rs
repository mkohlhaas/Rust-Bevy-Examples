...
impl PersistentGpuBufferable for Arc<[BvhNode]> {
  ...
  fn write_bytes_le(
    ...
    for (i, &node) in self.iter().enumerate() {
      let bytes: [u8; SIZE] = bytemuck::cast(node.offset_aabbs(base_meshlet_index, buffer_offset));
      buffer_slice
  ...
  fn write_bytes_le(
    ...
    for (i, meshlet) in self.iter().enumerate() {
      ...
      let bytes = bytemuck::cast::<_, [u8; size_of::<Meshlet>()]>(Meshlet {
        start_vertex_position_bit: meshlet.start_vertex_position_bit + vertex_position_offset,
        start_vertex_attribute_id: meshlet.start_vertex_attribute_id + vertex_attribute_offset,
