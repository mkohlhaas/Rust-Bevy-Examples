  ...
  C: LightProbeComponent,
{
  ...
  fn new(
    ...
    environment_map.id(image_assets).map(|id| LightProbeInfo {
      ...
      parallax_correction_bounds: environment_map.parallax_correction_bounds(&query_components),
      asset_id: id,
  ...
  fn camera_distance_sort_key(&self, view_transform: &GlobalTransform) -> FloatOrd {
    FloatOrd(
      (self.world_from_light.translation - view_transform.translation_vec3a()).length_squared(),
    )
  }
}
...
pub(crate) fn binding_arrays_are_usable(
  ...
  !cfg!(feature = "shader_format_glsl")
    ...
    && render_device.limits().max_storage_textures_per_shader_stage
      >= (STANDARD_MATERIAL_FRAGMENT_SHADER_MIN_TEXTURE_BINDINGS + MAX_VIEW_LIGHT_PROBES) as u32
