  ...
  fn build(&self, app: &mut App) {
    app
      .add_systems(
        Startup,
        spawn_gizmo_meshes
          .run_if(bevy_ecs::schedule::common_conditions::resource_exists::<TransformGizmoSettings>),
      )
      .add_systems(
        PostUpdate,
        (
...
fn update_gizmo_meshes(
  ...
  let (Some(materials_res), Some(settings), Some(state)) = (materials_res, settings, state) else {
    return;
  };
