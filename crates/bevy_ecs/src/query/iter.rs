...
use crate::{
  ...
  query::{
    ArchetypeFilter, ArchetypeQueryData, ContiguousQueryData, DebugCheckedUnwrap, IterQueryData,
    QueryState, SingleEntityQueryData, StorageId,
  },
  ...
  QueryManyIter<'w, 's, D, F, I>
{
  ...
  unsafe fn fetch_next_aliased_unchecked(
    ...
    for entity_borrow in entity_iter {
      ...
      if unsafe {
        ...
        // - set_archetype was called prior, `location.archetype_row` is an archetype index in range of the current archetype
        // - Caller ensures there are no conflicting items alive
        let item = unsafe { D::fetch(&query_state.fetch_state, fetch, entity, location.table_row) };
  ...
  > {
    ...
    // `QueryIter::new` ensures `world` is the same one used to initialize `query_state`.
    let query_lens =
      unsafe { query_lens_state.query_unchecked_manual(world) }.iter_many_inner(self.entity_iter);
    let mut keyed_query: Vec<_> = query_lens
      .map(|(key, entity)| (key, NeutralOrd(entity)))
      .collect();
    f(&mut keyed_query);
    ...
    D: IterQueryData,
  {
    if self.current_row > 0 {
      ...
      if self.is_dense {
        ...
        // SAFETY: This must have been called previously in `next` as `current_row > 0`
        let archetype_entity = unsafe { self.archetype_entities.get_unchecked(index as usize) };
        // SAFETY:
