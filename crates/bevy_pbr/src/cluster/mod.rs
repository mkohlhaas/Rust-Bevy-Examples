  ...
  pub fn min_size(buffer_binding_type: BufferBindingType) -> NonZero<u64> {
    match buffer_binding_type {
      ...
      BufferBindingType::Uniform => NonZero::try_from(
        u64::from(GpuClusteredLight::min_size()) * MAX_UNIFORM_BUFFER_CLUSTERABLE_OBJECTS as u64,
      )
      .unwrap(),
    }
  }
  ...
  pub fn clear(&mut self) {
    match &mut self.buffers {
      ViewClusterBuffers::Uniform {
        ...
        *clusterable_object_index_lists.get_mut().data = [UVec4::ZERO; Self::MAX_UNIFORM_ITEMS];
  ...
  fn push_offset_and_counts(&mut self, offset: usize, counts: &ClusterableObjectCounts) {
    match &mut self.buffers {
      ViewClusterBuffers::Uniform {
        ...
        let packed = pack_offset_and_counts(offset, counts.point_lights, counts.spot_lights);
