...
impl BundleInfo {
  ...
  pub(super) unsafe fn write_components<'a, T: DynamicBundle, S: BundleComponentStatus>(
    ...
    for required_component in required_components {
      ...
      unsafe {
        required_component.initialize(table, sparse_sets, change_tick, table_row, entity, caller);
      }
    }
  }
...
impl Bundles {
  ...
  pub(crate) fn init_dynamic_info(
    ...
    let (_, bundle_id) = self
      ...
      .or_insert_with(|| {
        let (id, storages) =
          initialize_dynamic_bundle(bundle_infos, storages, components, Vec::from(component_ids));
  ...
  pub(crate) fn init_component_info(
    ...
    let bundle_id = self
      ...
      .or_insert_with(|| {
        let (id, storage_type) =
          initialize_dynamic_bundle(bundle_infos, storages, components, vec![component_id]);
...
fn initialize_dynamic_bundle(
  ...
  let storage_types = component_ids
    .iter()
    .map(|&id| {
      components
        .get_info(id)
