...
use bevy::{
  ...
  render::{
    ...
    render_resource::{
      ...
      DepthBiasState, DepthStencilState, Face, FragmentState, MultisampleState, PipelineCache,
      PrimitiveState, PrimitiveTopology, RenderPassDescriptor, RenderPipelineDescriptor,
      SpecializedRenderPipeline, SpecializedRenderPipelines, StencilFaceState, StencilState,
      StoreOp, VertexFormat, VertexState, VertexStepMode,
    },
  ...
  fn specialize(&self, key: Self::Key) -> RenderPipelineDescriptor {
    ...
    RenderPipelineDescriptor {
      ...
      primitive: PrimitiveState {
        ...
        strip_index_format: BaseMeshPipelineKey::from_bits_retain(key.bits()).strip_index_format(),
        ..default()
      },
    ...
    app
      ...
      .add_systems(
        ...
        (
          ...
          batch_and_prepare_sorted_render_phase::<TransparentColoredMesh2d, ColoredMesh2dPipeline>
...
fn main_colored_transparent_pass_2d(
  ...
  let Some(transparent_phase) = transparent_phases.get(&extracted_view.retained_view_entity) else {
    return;
  };
...

  {
    ...
    if !transparent_phase.items.is_empty() {
      ...
      if let Err(err) = transparent_phase.render(&mut render_pass, world, view_entity) {
        error!("Error encountered while rendering the colored transparent 2D phase {err:?}");
      }
    }
  ...
  P: PhaseItem,
{
  ...
  fn render<'w>(
    ...
    let Some(bind_group) = &mesh2d_bind_group.value.get(&metadata_slab_id) else {
      return RenderCommandResult::Failure("The mesh2d bind group wasn't set in the render phase.");
    };
