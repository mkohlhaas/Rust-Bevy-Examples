          ...
          match dlss_wgpu::register_instance_extensions(
            ...
            Ok(_) => {
              ...
              if feature_support.ray_reconstruction_supported {
                additional_vulkan_features.insert::<DlssRayReconstructionSupported>();
              }
            }
            Err(_) => {}
          }
...
impl Plugin for DlssPlugin {
  fn build(&self, app: &mut App) {
    app
      .register_type::<Dlss<DlssSuperResolutionFeature>>()
...
pub trait DlssFeature: Reflect + Clone + Default {
  ...
  fn suggested_jitter(context: &Self::Context, frame_number: u32, render_resolution: UVec2)
    -> Vec2;
