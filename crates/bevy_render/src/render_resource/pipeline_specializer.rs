...
type VertexLayoutCache<S> =
  HashMap<VertexBufferLayout, HashMap<<S as SpecializedMeshPipeline>::Key, CachedRenderPipelineId>>;
...
impl<S: SpecializedMeshPipeline> SpecializedMeshPipelines<S> {
  ...
  pub fn specialize(
    ...
    fn specialize_slow<S>(
      ...
      key: S::Key,
      layout: &MeshVertexBufferLayoutRef,
      entry: VacantEntry<(MeshVertexBufferLayoutRef, S::Key), CachedRenderPipelineId, FixedHasher>,
    ) -> Result<CachedRenderPipelineId, SpecializedMeshPipelineError>
    where
