...
fn orbit_camera(
  ...
  for (mut camera_transform, mut orbit_camera) in &mut cameras {
    ...
    *camera_transform = Transform::from_translation(new_translation + sphere_transform.translation)
  ...
  match app_status.camera_mode {
    CameraMode::Orbit => {
      for (camera_entity, camera_transform) in &cameras_query {
        ...
        // coordinates.
        let relative_camera_position = camera_transform.translation - sphere_transform.translation;
...
fn set_help_text(app_status: &AppStatus, help_text_query: &mut Query<&mut Text, With<HelpText>>) {
  for mut ui_text in help_text_query {
    ...
    match app_status.camera_mode {
      CameraMode::Orbit => {
        help_text
          .push_str("Click and drag to orbit the camera\nUse the mouse wheel to zoom the camera\n");
      }
      CameraMode::Free => {
        help_text.push_str("Click and drag to rotate the camera\nUse WASDEQ to move the camera\n");
      }
    }
