  ...
  C: IterableCurve<f32> + Debug + Clone + Reflectable,
{
  ...
  fn apply(
    ...
    curve_evaluator.morph_target_count = Some(
      (curve_evaluator.stack_morph_target_weights.len() - prev_morph_target_weights_len) as u32,
    );
...
impl WeightsCurveEvaluator {
  fn combine(
    ...
    let stack_iter = self
      .stack_morph_target_weights
      .drain((self.stack_morph_target_weights.len() - self.morph_target_count.unwrap() as usize)..);
...
impl AnimationCurveEvaluator for WeightsCurveEvaluator {
  ...
  fn commit(&mut self, mut entity: AnimationEntityMut) -> Result<(), AnimationEvaluationError> {
    ...
    for (dest, src) in entity
      ...
      .ok_or_else(|| AnimationEvaluationError::ComponentNotPresent(TypeId::of::<MorphWeights>()))?
      .weights_mut()
      .iter_mut()
