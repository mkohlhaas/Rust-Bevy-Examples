    ...
    Action::Run {
      ...
      match (stop_frame, screenshot_frame, auto_stop_frame) {
        ...
        (_, 0, _) => {
          let mut file = File::create("example_showcase_config.ron").unwrap();
          file
            .write_all(format!("(events: [({stop_frame}, AppExit)])").as_bytes())
            .unwrap();
          extra_parameters.push("--features");
          extra_parameters.push("bevy_ci_testing");
        }
      ...
      let work_to_do = || {
        examples_to_run
          ...
          .filter(|example| !only_default_features || example.required_features.is_empty())
      ...
      if report_details {
        fs::create_dir(reports_path).expect("Failed to create example-showcase-reports directory");
      }
      ...
      for to_run in work_to_do() {
        ...
        if report_details || show_logs {
          ...
          if report_details {
            let mut file = File::create(format!("{reports_path}/{example}.log")).unwrap();
