...
use bevy_render::{
  ...
  camera::{
    extract_cameras, DirtySpecializationSystems, DirtyWireframeSpecializations, ExtractedCamera,
    PendingQueues,
  },
...
pub fn specialize_wireframes(
  ...
  for (view, visible_entities) in &views {
    ...
    for (_, visible_entity) in dirty_wireframe_specializations.iter_to_specialize(
      ...
      let pipeline_id = pipelines.specialize(&pipeline_cache, &pipeline, mesh_key, &mesh.layout);
...
fn queue_wireframes(
  ...
  for (view, visible_entities) in &mut views {
    ...
    for &main_entity in
      dirty_wireframe_specializations.iter_to_dequeue(view.retained_view_entity, visible_entities)
    {
