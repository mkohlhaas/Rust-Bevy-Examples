...
pub struct ReflectAsset {
  ...
  insert: fn(&mut World, UntypedAssetId, &dyn PartialReflect) -> Result<(), InvalidGenerationError>,
  len: fn(&World) -> usize,
    ...
    if let Some(untyped_handle) = value_reflect.downcast_ref::<UntypedHandle>() {
      ...
      return Ok(Ok(
        TypedHandleReference {
          ...
          reference: handle_reference_from_handle(untyped_handle, self.ephemeral_handle_behavior)
...
impl ReflectDeserializerProcessor for HandleDeserializeProcessor<'_> {
    ...
    D: Deserializer<'de>,
  {
    if registration.type_id() == TypeId::of::<UntypedHandle>() {
      ...
      let Some(asset_type) = registry.get_with_type_path(&typed_handle_reference.asset_type) else {
...
mod tests {
  ...
  use crate::{
    ...
    Asset, AssetApp, AssetServer, Assets, DirectAssetAccessExt, EphemeralHandleBehavior, Handle,
    HandleDeserializeProcessor, HandleSerializeProcessor, LoadedUntypedAsset, ReflectAsset,
    UntypedHandle,
  };
  ...
  fn roundtrip_reflect_serialize_handles() {
    ...
    run_app_until(&mut app, |_| {
      (asset_server.is_loaded(&stuff.typed) && asset_server.is_loaded(&stuff.untyped)).then_some(())
    });
