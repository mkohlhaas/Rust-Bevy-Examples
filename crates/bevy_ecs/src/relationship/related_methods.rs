...
impl<'w> EntityWorldMut<'w> {
  ...
  pub fn replace_related_with_difference<R: Relationship>(
    ...
    #[cfg(debug_assertions)]
    {
      ...
      let entities_to_unrelate = EntityHashSet::from_iter(entities_to_unrelate.iter().copied());
    ...
    if size_of::<R>() > size_of::<Entity>() {
      ...
      let modified = self.world_scope(|world| {
        let modified = DeferredWorld::from(&mut *world)
          .modify_component_with_relationship_hook_mode::<R, _>(this, relationship_hook_mode, |r| {
            r.set_risky(entity)
          })
