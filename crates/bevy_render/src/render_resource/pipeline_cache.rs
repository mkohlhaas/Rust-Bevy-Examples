...
fn load_module(
  ...
  let shader_module = WgpuWrapper::new(match validate_shader {
    ValidateShader::Enabled => render_device.create_and_validate_shader_module(module_descriptor),
    ...
    ValidateShader::Disabled => unsafe { render_device.create_shader_module(module_descriptor) },
  });
    ...
    match &mut cached_pipeline.state {
      ...
      CachedPipelineState::Err(err) => match err {
        ...
        ShaderCacheError::ProcessShaderError(err) => {
          let error_detail = err.emit_to_string(&self.shader_cache.lock().unwrap().composer);
