...
fn on_pointer_drag(
  ...
  let Some(local_point) = transform.try_inverse().map(|inverse| {
    inverse.transform_point2(drag.pointer_location.position * target.scale_factor() / ui_scale.0)
  }) else {
    return;
  };
...
pub(crate) fn text_input_autoscroll_system(
  ...
  let Some(local_point) = transform
    .try_inverse()
    .map(|inverse| inverse.transform_point2(pointer_position * target.scale_factor() / ui_scale.0))
  else {
    return;
  };
...
impl Plugin for EditableTextInputPlugin {
  fn build(&self, app: &mut App) {
    app
      ...
      .add_systems(
        ...
        (
          ...
          listen_for_ime_input_when_text_input_focused.in_set(ImeSystems::ToggleWindowIMEInput),
        )
          .after(InputSystems)
    ...
    app
      .register_required_components::<EditableText, Node>()
...
mod tests {
  ...
  fn steam_osk_unidentified_keys_insert_produced_text() {
    let mut app = App::new();
    app
      .add_plugins((InputPlugin, InputDispatchPlugin))
