...
pub fn create_bindless_bind_group_layout_entries(
  ...
  for &(resource_type, ref binding_number) in BINDING_NUMBERS.iter() {
    ...
    let Some(binding_type) = (match resource_type {
      ...
      BindlessResourceType::SamplerNonFiltering => Some(sampler(SamplerBindingType::NonFiltering)),
      ...
      BindlessResourceType::Texture2dArray => Some(texture_2d_array(TextureSampleType::Float {
        filterable: true,
      })),
