...
impl PluginGroupBuilder {
  ...
  // already present
  fn upsert_plugin_entry_state(&mut self, key: TypeId, plugin: PluginEntry, added_at_index: usize) {
  ...
  pub fn add_before<Target: Plugin>(self, plugin: impl Plugin) -> Self {
    self
      .try_add_before_overwrite::<Target, _>(plugin)
      .unwrap_or_else(|_| {
        panic!(
  ...
  pub fn add_after<Target: Plugin>(self, plugin: impl Plugin) -> Self {
    self
      .try_add_after_overwrite::<Target, _>(plugin)
      .unwrap_or_else(|_| {
        panic!(
          "Plugin does not exist in group: {}.",
          core::any::type_name::<Target>()
  ...
  pub fn finish(mut self, app: &mut App) {
    for ty in &self.order {
        ...
        && entry.enabled
      {
        ...
        if let Err(AppError::DuplicatePlugin { plugin_name }) = app.add_boxed_plugin(entry.plugin) {
          panic!(
...
mod tests {
  ...
  fn add_overwrite() {
    ...
    let Ok(group) = group.try_add_after_overwrite::<PluginA, _>(PluginWithData(0xdeadbeef)) else {
      panic!("PluginA wasn't in group");
    };
    assert_eq!(
      get_plugin::<PluginWithData>(&group, id),
