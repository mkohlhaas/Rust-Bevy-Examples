...
impl WinitWindows {
  ...
  pub fn create_window(
    ...
    winit_window_attributes = match window.mode {
      ...
      WindowMode::Fullscreen(monitor_selection, video_mode_selection) => winit_window_attributes
        .with_fullscreen(Some(resolve_exclusive_fullscreen(
    ...
    #[cfg(target_os = "windows")]
    {
      ...
      winit_window_attributes = winit_window_attributes.with_clip_children(window.clip_children);
    }
    ...
    #[cfg(target_os = "ios")]
    {
      ...
      winit_window_attributes =
        winit_window_attributes.with_prefers_status_bar_hidden(window.prefers_status_bar_hidden);
    }
    ...
    if let Some(name) = &window.name {
      ...
      {
        winit_window_attributes = winit::platform::wayland::WindowAttributesExtWayland::with_name(
          winit_window_attributes,
          name.clone(),
          "",
        );
  ...
  match position {
    ...
    WindowPosition::Centered(monitor_selection) => {
      ...
      if let Some(monitor) = maybe_monitor {
        ...
        let (width, height): (u32, u32) = LogicalSize::new(resolution.width(), resolution.height())
          .to_physical::<u32>(scale_factor)
          .into();

        let position = PhysicalPosition {
          x: screen_size.width.saturating_sub(width) as f64 / 2. + monitor.position().x as f64,
          y: screen_size.height.saturating_sub(height) as f64 / 2. + monitor.position().y as f64,
        };
