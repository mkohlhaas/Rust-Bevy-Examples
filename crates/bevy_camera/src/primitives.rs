...
impl MeshAabb for Mesh {
  fn get_aabb(&self) -> Option<Aabb> {
    ...
    let Ok(VertexAttributeValues::Float32x3(values)) = self.try_attribute(Mesh::ATTRIBUTE_POSITION)
    else {
      return None;
    };
...
mod tests {
  ...
  fn sphere_intersects_obb_rotated_zeros() {
    ...
    let transform =
      Affine3A::from_rotation_translation(Quat::from_rotation_y(PI), Vec3::new(5.0, 0.0, 0.0));
