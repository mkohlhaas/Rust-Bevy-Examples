...
mod spot_light;
pub use spot_light::{
  orthonormalize, spot_light_clip_from_view, spot_light_world_from_view, update_spot_light_frusta,
...
pub mod prelude {
  ...
  pub use crate::{
    ...
    GeneratedEnvironmentMapLight, GlobalAmbientLight, LightProbe, PointLight, RectLight, SpotLight,
  };
          ...
          |cubemap_visible_entities_local_queue,
           (
            ...
            if has_visibility_range
              && visible_entity_ranges.is_some_and(|visible_entity_ranges| {
                shadow_lod_origin.is_none_or(|shadow_lod_origin| {
                  !visible_entity_ranges.entity_is_in_range_of_view(entity, shadow_lod_origin)
                })
              })
            {
              return;
            ...
            if let (Some(aabb), Some(transform)) = (maybe_aabb, maybe_transform) {
              ...
              if !has_no_frustum_culling && !light_sphere.intersects_obb(aabb, &model_to_world) {
                return;
              }
          ...
          |spot_visible_entities_local_queue,
           (
            ...
            if let (Some(aabb), Some(transform)) = (maybe_aabb, maybe_transform) {
              ...
              if has_no_frustum_culling || frustum.intersects_obb(aabb, &model_to_world, true, true)
              {
                view_visibility.set_visible();
...
pub fn get_shadow_lod_origin(
  ...
  entities.extend(camera_query.iter().filter_map(
    |(main_entity, render_target)| match *render_target {
