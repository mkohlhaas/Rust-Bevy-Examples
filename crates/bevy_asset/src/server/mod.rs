...
use crate::{
  folder::LoadedFolder,
  io::{
    AssetReaderError, AssetSource, AssetSourceEvent, AssetSourceId, AssetSources, AssetWriterError,
...
impl AssetServer {
  ...
  pub async fn get_path_asset_loader<'a>(
    ...
    let error = || {
      ...
      extensions
        .extend(AssetPath::iter_secondary_extensions(full_extension).map(ToString::to_string));
  ...
  pub async fn get_asset_loader_with_asset_type<A: Asset>(
    ...
    self
      .get_asset_loader_with_asset_type_id(TypeId::of::<A>())
      .await
  }
  ...
  #[deprecated(note = "Use `asset_server.load_builder().override_unapproved().load(path)` instead")]
    ...
    let task = IoTaskPool::get().spawn(async move {
        ...
        .map(|h| h.expect("handle must be returned, since we didn't pass in an input handle"))
      {
        Ok(handle) => server.send_asset_event(InternalAssetEvent::Loaded {
          index,
          loaded_asset: LoadedAsset::new_with_dependencies(LoadedUntypedAsset { handle }).into(),
        }),
        Err(err) => {
          error!("{err}");
  ...
  pub(crate) fn load_asset<A: Asset>(&self, asset: impl Into<LoadedAsset<A>>) -> Handle<A> {
    ...
    self
      .load_asset_untyped(None, erased_loaded_asset)
      .typed_debug_checked()
  }
  ...
  pub(crate) fn load_asset_untyped(
    ...
    let handle = if let Some(path) = path {
      ...
      self
        .write_infos()
        .create_loading_handle_untyped(loaded_asset.asset_type_id(), loaded_asset.asset_type_name())
    };
    self.send_asset_event(InternalAssetEvent::Loaded {
  ...
  pub fn load_folder<'a>(&self, path: impl Into<AssetPath<'a>>) -> Handle<LoadedFolder> {
    ...
    let (handle, should_load) =
      self
        .write_infos()
        .get_or_create_path_handle(path.clone(), HandleLoadingMode::Request, None);
    if !should_load {
      return handle;
    }
      ...
      .spawn(async move {
        ...
        .await
        {
          Ok(_) => server.send_asset_event(InternalAssetEvent::Loaded {
            index,
            loaded_asset: LoadedAsset::new_with_dependencies(LoadedFolder { handles }).into(),
          }),
          Err(err) => {
  ...
  pub fn get_id_handle<A: Asset>(&self, id: AssetId<A>) -> Option<Handle<A>> {
    self
      .get_id_handle_untyped(id.untyped())
      ...
      match asset_reader.read_meta(asset_path.path()).await {
        Ok(new_meta_reader) => {
          ...
          let minimal: AssetMetaMinimal =
            ron::de::from_bytes(&meta_bytes).map_err(|e| AssetLoadError::DeserializeMeta {
          ...
          let meta =
            loader
              ...
              .map_err(|e| AssetLoadError::DeserializeMeta {
                path: asset_path.clone_owned(),
                error: e.into(),
              })?;
  ...
  fn wait_for_asset_id_poll_fn(
    ...
    match (&info.load_state, &info.rec_dep_load_state) {
      ...
      (LoadState::Failed(error), _) => Poll::Ready(Err(WaitForAssetError::Failed(error.clone()))),
...
impl<'a> LoadBuilder<'a> {
  ...
  pub fn load<'b, A: Asset>(self, asset_path: impl Into<AssetPath<'b>>) -> Handle<A> {
    self
      .load_typed_internal(TypeId::of::<A>(), Some(type_name::<A>()), asset_path.into())
      .typed_unchecked()
  }
...
pub fn handle_internal_asset_events(world: &mut World) {
  world.resource_scope(|world, server: Mut<AssetServer>| {
    ...
    for event in server.data.asset_event_receiver.try_iter() {
      match event {
        InternalAssetEvent::Loaded {
          ...
          infos.process_asset_load(index, loaded_asset, world, &server.data.asset_event_sender);
        }
    ...
    let mut reload_parent_folders =
      |path: &PathBuf, source: &AssetSourceId<'static>, infos: &mut AssetInfos| {
        ...
        for parent in path.ancestors().skip(1) {
          let parent_asset_path = AssetPath::from(parent.to_path_buf()).with_source(source.clone());
    ...
    let mut reload_path = |path: PathBuf, source: &AssetSourceId<'static>, infos: &AssetInfos| {
