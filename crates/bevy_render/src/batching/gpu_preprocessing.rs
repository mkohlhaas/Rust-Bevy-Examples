...
use crate::{
  ...
  render_phase::{
    ...
    RenderMultidrawableBatchSet, SortedPhaseItem, SortedRenderPhase,
  ...
  F: GetBatchData,
{
    ...
    I: CachedRenderPipelinePhaseItem + SortedPhaseItem,
  {
    ...
    *batch_extra_index = match self.indirect_parameters_index_range {
      Some(ref indirect_parameters_index_range) => PhaseItemExtraIndex::IndirectParametersIndex {
  ...
  GFBD: GetFullBatchData,
{
  ...
  for (extracted_view, no_indirect_drawing, gpu_occlusion_culling) in &mut views {
    ...
    for current_index in 0..phase.items.len() {
      ...
      let current_batch_input_index = GFBD::get_index_and_compare_data(&system_param_item, entity);
      ...
      match can_batch {
        SortedPhaseItemBatchability::BreakBatchSet => {
          ...
          let indirect_parameters_index =
            phase_indirect_parameters_buffers.allocate(no_indirect_drawing, item_is_indexed);
          ...
          batch_set = Some(SortedRenderBatchSet {
            ...
            indirect_parameters_index_range: indirect_parameters_index.map(|i| i..(i + 1)),
            meta: current_meta,
          });
        }
        ...
        SortedPhaseItemBatchability::BreakBatch => {
          ...
          let maybe_indirect_parameters_index =
            phase_indirect_parameters_buffers.allocate(no_indirect_drawing, item_is_indexed);
  ...
  for (extracted_view, no_indirect_drawing, gpu_occlusion_culling) in &mut views {
    ...
    for (key, unbatchables) in &mut phase.unbatchable_meshes {
      ...
      for main_entity in unbatchables.entities.keys() {
        let Some(input_index) = GFBD::get_binned_index(&system_param_item, *main_entity) else {
          continue;
        };
        let output_index = data_buffer.add() as u32;
          ...
          Some(ref mut batch) => {
            ...
            work_item_buffer.push(
              ...
              PreprocessWorkItem {
                ...
                output_or_indirect_parameters_index: match (no_indirect_drawing, &batch.extra_index)
                {
                  (true, _) => output_index,
                  (
                    false,
          ...
          BinnedRenderPhaseBatchSets::MultidrawIndirect(ref mut vec) => {
            ...
            vec.push(BinnedRenderPhaseBatchSet {
              ...
              index: phase_indirect_parameters_buffers
                .buffers
                .batch_set_count(key.0.indexed()) as u32,
              // Unused.
  ...
  GFBD: GetFullBatchData,
{
    ...
    IP: Clone + ShaderSize + WriteInto,
  {
    ...
    let first_metadata_index = mesh_class_buffers.metadata.push_multiple_uninit(bin_count) as u32;
  ...
  for extracted_view in &mut views {
    ...
    for (batch_set_key, batch_set) in view_binned_render_phase.multidrawable_meshes.iter_mut() {
      ...
      let Some(scene_unpacking_metadata) =
        representative_entity_to_batch_set_scene_unpacking_metadata.get(&representative_entity)
      else {
        continue;
      };
      ...
      let gpu_bin_unpacking_metadata_index =
        bin_unpacking_buffers
          .bin_unpacking_metadata
          .push(GpuBinUnpackingMetadata {
            base_output_work_item_index: scene_unpacking_metadata.base_output_work_item_index,
            base_indirect_parameters_index: scene_unpacking_metadata.base_indirect_parameters_index,
      ...
      let gpu_uniform_allocation_metadata_index = bin_unpacking_buffers
        ...
        .push(GpuUniformAllocationMetadata {
          ...
          bin_count,
          first_indirect_parameters_index: scene_unpacking_metadata.first_indirect_parameters_index,
          first_output_mesh_uniform_index: scene_unpacking_metadata.first_output_mesh_uniform_index,
          pad: [0; _],
        });

      let (Some(gpu_bin_unpacking_metadata_index), Some(gpu_uniform_allocation_metadata_index)) = (
      ...
      let job = SceneUnpackingJob {
        ...
        bin_index_to_bin_metadata_index_buffer: bin_index_to_bin_metadata_index_buffer.clone(),
        bin_unpacking_metadata_index: BinUnpackingMetadataIndex(gpu_bin_unpacking_metadata_index),
