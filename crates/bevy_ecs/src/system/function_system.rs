...
impl<Param: SystemParam> SystemState<Param> {
  ...
  unsafe fn fetch<'w, 's>(
    ...
    // SAFETY: The invariants are upheld by the caller.
    let param = unsafe { Param::get_param(&mut self.param_state, &self.meta, world, change_tick) }?;
  ...
  F: SystemParamFunction<Marker>,
{
  ...
  fn new(func: F, system_meta: SystemMeta, state: Option<FunctionSystemState<F::Param>>) -> Self {
    Self {
      func,
      #[cfg(feature = "hotpatching")]
      current_ptr: subsecond::HotFn::current(<F as SystemParamFunction<Marker>>::run).ptr_address(),
      state,
      system_meta,
      marker: PhantomData,
    }
