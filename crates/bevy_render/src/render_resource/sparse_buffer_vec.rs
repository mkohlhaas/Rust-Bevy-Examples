...
fn update_sparse_buffers(
  ...
  let mut sparse_buffer_update_pass = command_encoder.begin_compute_pass(&ComputePassDescriptor {
  ...
  for sparse_buffer_update_job in sparse_buffer_update_jobs.iter() {
    ...
    sparse_buffer_update_pass.set_bind_group(0, &sparse_buffer_update_bind_group.bind_group, &[]);
    sparse_buffer_update_pass.dispatch_workgroups(sparse_buffer_update_job.workgroup_count(), 1, 1);
  }
...
impl SparseBufferStagingBuffers {
  ...
  fn new(label: &str, element_word_size: u32) -> SparseBufferStagingBuffers {
    let mut source_data_buffer = RawBufferVec::new(BufferUsages::COPY_DST | BufferUsages::STORAGE);
  ...
  fn should_perform_full_reupload(&self, changed_element_count: u32, buffer_length: usize) -> bool {
  ...
  T: AtomicPod,
{
  ...
  fn should_perform_full_reupload(&self, render_device: &RenderDevice) -> bool {
    if self.needs_full_reupload || render_device.limits().max_storage_buffers_per_shader_stage < 3 {
      return true;
    }
  ...
  fn prepare_sparse_upload(&mut self, render_device: &RenderDevice, render_queue: &RenderQueue) {
    ...
    for (summary_word_index, atomic_summary_word) in self.summary.iter().enumerate() {
      ...
      for summary_bit_offset in BitIter::new(summary_word) {
        ...
        for dirty_bit_offset in BitIter::new(dirty_word) {
          let element_index = dirty_word_index * BITS_PER_WORD as usize + dirty_bit_offset as usize;
    ...
    if self.sparse_update_scheduled {
      self
        .staging_buffers
        .write_buffers(&mut self.metadata_uniform, render_device, render_queue);
    }
  }
  ...
  fn from_world(world: &mut World) -> Self {
    world.resource_scope::<SpecializedComputePipelines<SparseBufferUpdatePipelines>, _>(
      |world, mut specialized_sparse_buffer_update_pipelines| {
        ...
        let sparse_buffer_update_pipelines = world.resource::<SparseBufferUpdatePipelines>();
