...
use bevy_ui::{
  prelude::BorderRect, ui_layout_system, BackgroundColor, BorderColor, BorderRadius, ComputedNode,
...
fn scrollbar_on_pointer_down(
  ...
  if q_thumb.contains(ev.entity) {
    ...
    // back.
    let visible_size =
      (scroll_content.size() - scroll_content.scrollbar_size) * scroll_content.inverse_scale_factor;
    ...
    if drag.dragging {
      ...
      match scrollbar.orientation {
        ControlOrientation::Horizontal => {
          ...
          scroll_pos.x =
            (drag.drag_origin + (distance.x * content_size.x) / scrollbar_size.x).clamp(0., range);
        }
        ControlOrientation::Vertical => {
          let range = (content_size.y - visible_size.y).max(0.);
          scroll_pos.y =
            (drag.drag_origin + (distance.y * content_size.y) / scrollbar_size.y).clamp(0., range);
        }
      };
    }
  }
...
pub(crate) fn update_scrollbar_thumb(
  ...
  for (scrollbar, scrollbar_node, scrollbar_transform, children) in q_scrollbar.iter() {
    ...
    let visible_size =
      (scroll_area.1.size() - scroll_area.1.scrollbar_size) * scroll_area.1.inverse_scale_factor;
    ...
    for child in children {
      let Ok((thumb, thumb_transform, target_info, mut thumb_node, mut thumb_global_transform)) =
      ...
      if thumb_node.size.x < resolved_border.min_inset.x + resolved_border.max_inset.x {
        let r = thumb_node.size.x / (resolved_border.min_inset.x + resolved_border.max_inset.x);
      ...
      if thumb_node.size.y < resolved_border.min_inset.y + resolved_border.max_inset.y {
        let r = thumb_node.size.y / (resolved_border.min_inset.y + resolved_border.max_inset.y);
...
impl Plugin for ScrollbarPlugin {
  fn build(&self, app: &mut App) {
    app
      .add_observer(scrollbar_on_pointer_down)
