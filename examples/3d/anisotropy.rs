...
fn setup(mut commands: Commands, asset_server: Res<AssetServer>, app_status: Res<AppStatus>) {
  ...
  commands.spawn((
    WorldAssetRoot(asset_server.load("models/AnisotropyBarnLamp/AnisotropyBarnLamp.gltf#Scene0")),
...
fn rotate_camera(
  ...
  for mut transform in camera.iter_mut() {
    *transform =
      Transform::from_translation(Quat::from_rotation_y(now).mul_vec3(CAMERA_INITIAL_POSITION))
