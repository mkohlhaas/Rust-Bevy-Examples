use crate::_bsn::types::{
  Bsn, BsnConstructor, BsnEntry, BsnFields, BsnFnArg, BsnFnArgs, BsnListRoot, BsnRelatedSceneList,
  BsnRoot, BsnScene, BsnSceneFn, BsnSceneListItem, BsnSceneListItems, BsnType, BsnValue,
};
...
impl BsnType {
  ...
  fn process_enum_field(
    ...
    if let Some(
      value @ (BsnValue::Ident(_) | BsnValue::Expr(_) | BsnValue::Closure(_) | BsnValue::Tuple(_)),
    ) = value
    {
      let ident = ctx.hoisted_expressions.hoist(value);
