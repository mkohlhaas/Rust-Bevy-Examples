...
fn asset_processor_transforms_asset_default_processor() {
  ...
  type CoolTextProcessor =
    LoadTransformAndSave<CoolTextLoader, RootAssetTransformer<AddText, CoolText>, CoolTextSaver>;
  app
...
impl AssetLoader for FakeGltfLoader {
  ...
  async fn load(
    ...
    ron::de::from_bytes(&bytes).map_err(|err| Error::new(ErrorKind::InvalidData, err.to_string()))
  }
...
impl AssetSaver for FakeBsnSaver {
  ...
  async fn save(
    ...
    let ron_string = ron::ser::to_string_pretty(asset.get(), PrettyConfig::new().new_line("\n"))
      .map_err(|err| Error::new(ErrorKind::InvalidData, err))?;
...
fn nested_loads_of_processed_asset_reprocesses_on_reload() {
  ...
  type NesterProcessor =
    LoadTransformAndSave<NesterLoader, RootAssetTransformer<AddTextToNested, Nester>, NesterSaver>;
  app
    .init_asset::<Nester>()
...
fn only_reprocesses_wrong_hash_on_startup() {
  ...
  default_source_dir.insert_asset_text(source_changed_asset, &serialize_as_cool_text("DIFFERENT"));
...
fn writes_long_default_meta_for_ambiguous_processor() {
  ...
  app
    .register_asset_processor(CoolTextProcessor1::new(
      RootAssetTransformer::new(AddText("blah".to_string())),
      CoolTextSaver,
    ))
