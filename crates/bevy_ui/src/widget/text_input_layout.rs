...
use bevy_text::{
  ...
  FontSize, GlyphCacheKey, LayoutCx, LineBreak, LineHeight, PositionedGlyph, RemSize, RunGeometry,
  ScaleCx, TextBrush, TextFont, TextLayout, TextLayoutInfo, TextLineYBounds,
};
...
fn query_family<'a, 'b>(
  ...
  match family {
    ...
    parley::FontFamilyName::Generic(generic) => parley::fontique::QueryFamily::Generic(*generic),
  }
}
  ...
  *cursor_timer += time.delta();
  ...
  ) in input_field_query.iter_mut()
  {
          ...
          match item {
            PositionedLayoutItem::GlyphRun(glyph_run) => {
              ...
              for glyph in glyph_run.positioned_glyphs() {
                ...
                .unwrap_or_else(|| {
                  let font_ref =
                    FontRef::from_index(font_data.data.as_ref(), font_data.index as usize).unwrap();
                ...
                && run_text_range.end > cr.start
              {
                ...
                if let Some(start_x) = underline_start_x {
                  info.preedit_underline_rects.push(Rect {
                    ...
                    max: Vec2::new(underline_end_x, underline_y + underline_thickness),
                  });
                }
              }
              ...
              info.run_geometry.push(RunGeometry {
                section_index: brush.section_index,
                bounds: Rect {
                  min: Vec2::new(glyph_run.offset(), line.metrics().block_min_coord),
                  max: Vec2::new(
    ...
    if is_focused {
      ...
      info.cursor = driver
        .editor
        .cursor_geometry(cursor_width * text_font.font_size.eval(target.logical_size(), rem_size.0))
