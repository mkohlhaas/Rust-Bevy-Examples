...
fn setup(
  ...
  commands.spawn((
    ...
    PointLight {
      color: Color::from(LinearRgba::from(ANTIQUE_WHITE).mix(&LinearRgba::from(ORANGE_RED), 0.2)),
      intensity: 4_000.0,
      radius: 0.2,
      range: 5.0,
...
fn example_control_system(
  ...
  for (material_handle, controls) in &controllable {
    ...
    if controls.color && randomize_colors {
      material.base_color = Color::srgba(random(), random(), random(), material.base_color.alpha());
    }
  }

  let (camera_entity, mut transmission, mut camera_transform, depth_prepass, temporal_jitter, hdr) =
    camera.into_inner();
