...
pub fn queue_ui_slices(
  ...
  for (main_entity, subslices) in extracted_ui_slicers.slices.iter() {
    for (render_entity, extracted_slicer) in subslices.iter() {
      let Ok(default_camera_view) = render_views.get_mut(extracted_slicer.extracted_camera_entity)
      else {
        continue;
      };
      ...
      for item_index in 0..ui_phase.items.len() {
          ...
          .and_then(|subslices| subslices.get(&item.entity()))
        {
              ...
              image_bind_groups
                ...
                .or_insert_with(|| {
                  render_device.create_bind_group(
                    "ui_texture_slice_image_layout",
                    &pipeline_cache.get_bind_group_layout(&texture_slicer_pipeline.image_layout),
          ...
          let positions = QUAD_VERTEX_POSITIONS
            .map(|pos| (texture_slices.transform.transform_point2(pos * rect_size)).extend(0.));
