...
pub fn extract_ui_material_nodes<M: UiMaterial>(
  ...
  for (entity, computed_node, stack_index, transform, handle, inherited_visibility, clip, camera) in
    uinode_query.iter().chain(
      nodes_to_reextract
        .into_iter()
...
impl<M: UiMaterial> RenderAsset for PreparedUiMaterial<M> {
  ...
  fn prepare_asset(
    ...
    (render_device, pipeline_cache, pipeline, material_param): &mut SystemParamItem<Self::Param>,
    _: Option<&Self>,
  ...
  M::Data: PartialEq + Eq + Hash + Clone,
{
  ...
  for (main_entity, extracted_sub_uinodes) in extracted_uinodes.uinodes.iter() {
    for (render_entity, extracted_uinode) in extracted_sub_uinodes.iter() {
      ...
      let Ok(default_camera_view) = render_views.get_mut(extracted_uinode.extracted_camera_entity)
      else {
        continue;
      };
      ...
      if transparent_phase.items.capacity() < extracted_uinodes.uinodes.len() {
        transparent_phase
          .items
          .reserve_exact(extracted_uinodes.uinodes.len() - transparent_phase.items.capacity());
      }
