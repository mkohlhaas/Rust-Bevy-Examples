...
impl Plugin for EnvironmentMapGenerationPlugin {
  ...
  fn finish(&self, app: &mut App) {
    ...
    app
      .add_plugins(SyncComponentPlugin::<GeneratedEnvironmentMapLight, Self>::default())
    ...
    render_app
      ...
      .add_systems(
        Render,
        (
          prepare_generated_environment_map_bind_groups.in_set(RenderSystems::PrepareBindGroups),
...
pub fn initialize_generated_environment_map_resources(
  ...
  let mips = texture_storage_2d_array(TextureFormat::Rgba16Float, StorageTextureAccess::WriteOnly);
    ...
    let downsampling = BindGroupLayoutDescriptor::new(
      ...
      &BindGroupLayoutEntries::sequential(
        ...
        (
          ...
          mips, // 4
          mips, // 5
          texture_storage_2d_array(TextureFormat::Rgba16Float, StorageTextureAccess::ReadWrite), // 6
          mips, // 7
          mips, // 8
          mips, // 9
          mips, // 10
...
pub fn prepare_generated_environment_map_bind_groups(
  ...
  for (entity, textures, env_map_light) in &light_probes {
    ...
    let irradiance_map = env_map_light
      .diffuse_map
      .texture
      .create_view(&TextureViewDescriptor {
        dimension: Some(TextureViewDimension::D2Array),
...
pub fn filtering_system(
  ...
  let Some(irradiance_pipeline) = pipeline_cache.get_compute_pipeline(pipelines.irradiance) else {
    return;
  };
