...
use bevy_render::{
  ...
  render_resource::{
    ...
    ShaderType, SpecializedRenderPipeline, SpecializedRenderPipelines, StoreOp, TextureDescriptor,
  ...
  fn build(&self, app: &mut App) {
    ...
    render_app
      ...
      .add_systems(
        Render,
        (
          configure_depth_of_field_view_targets.ambiguous_with(RenderSystems::PrepareViews),
  ...
  for pipeline_render_info in view_pipelines.pipeline_render_info().iter() {
    ...
    let view_bind_group = if pipeline_render_info.is_dual_input {
      ...
      ctx.render_device().create_bind_group(
        ...
        &BindGroupEntries::sequential((view_uniforms_binding, depth_view, postprocess.source)),
      )
    };
