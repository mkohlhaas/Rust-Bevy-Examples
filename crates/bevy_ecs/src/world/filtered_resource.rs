...
impl<'w, 's> FilteredResources<'w, 's> {
  ...
  pub fn get<R: Resource>(&self) -> Result<Ref<'w, R>, ResourceFetchError> {
    ...
    Ok(Ref {
      ...
      ticks: unsafe { ComponentTicksRef::from_tick_cells(ticks, self.last_run, self.this_run) },
    })
  }
...
impl<'w, 's> FilteredResourcesMut<'w, 's> {
  ...
  unsafe fn get_mut_by_id_unchecked(
    ...
    Ok(MutUntyped {
      ...
      // SAFETY: We have exclusive access to the underlying storage.
      ticks: unsafe { ComponentTicksMut::from_tick_cells(ticks, self.last_run, self.this_run) },
    })
  }
}
