use crate::{
  ...
  bundle::Bundle,
  change_detection::{
    ComponentTicksMut, ComponentTicksRef, ContiguousComponentTicksMut, ContiguousComponentTicksRef,
    ContiguousMut, ContiguousRef, MaybeLocation, Tick,
  },
  ...
  unsafe fn fetch<'w, 's>(
    ...
    Some(fetch.components.extract(
      ...
      |sparse_set| {
        ...
        Ref {
          value: component.deref(),
          ticks: ComponentTicksRef::from_tick_cells(ticks, fetch.last_run, fetch.this_run),
        }
      },
    ))
  }
    ...
    Some(fetch.components.extract(
      ...
      |sparse_set| {
        ...
        Mut {
          ...
          ticks: ComponentTicksMut::from_tick_cells(ticks, fetch.last_run, fetch.this_run),
        }
      },
    ))
  }
  ...
  for NestedQuery<D, F>
{
  ...
  unsafe fn fetch<'w, 's>(
    ...
    unsafe {
      Some(state.query_unchecked_manual_with_ticks(fetch.world, fetch.last_run, fetch.this_run))
    }
  }
