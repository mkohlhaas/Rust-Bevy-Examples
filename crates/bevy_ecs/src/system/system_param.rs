  ...
  for Single<'a, 'b, D, F>
{
  ...
  unsafe fn get_param<'w, 's>(
    ...
    match query.single_inner() {
      ...
      Err(QuerySingleError::NoEntities(_)) => Err(SystemParamValidationError::skipped::<Self>(
...
unsafe impl<'a, T: 'static> SystemParam for NonSend<'a, T> {
  ...
  unsafe fn get_param<'w, 's>(
    ...
    let (ptr, ticks) = world
      ...
      .ok_or_else(|| SystemParamValidationError::invalid::<Self>("Non-send data not found"))?;
    Ok(NonSend {
      value: ptr.deref(),
...
unsafe impl SystemParam for DynSystemParam<'_, '_> {
  ...
  unsafe fn get_param<'world, 'state>(
    ...
    Ok(unsafe { DynSystemParam::new(state.0.as_mut(), world, system_meta.clone(), change_tick) })
  }
