...
fn spawn_decal(
  ...
  commands.spawn((
    ...
    ExampleDecal {
      size: decal_size,
      state: ExampleDecalState::AnimatingIn(Timer::new(DECAL_ANIMATE_IN_DURATION, TimerMode::Once)),
    },
  ));
}
...
fn animate_decals(
  ...
  for (decal_entity, mut example_decal, mut decal_transform) in decals_query.iter_mut() {
    ...
    decal_transform.scale = Vec3::splat(example_decal.size * new_decal_scale_factor).with_z(1.0);
  }
}
...
fn update_radio_buttons(
  ...
  for (entity, image, has_text, sender) in widgets.iter_mut() {
    ...
    let selected = match **sender {
      AppSetting::EmissiveDecals(emissive_decals) => emissive_decals == app_status.emissive_decals,
    };
