use crate::{
  ...
  query::{
    has_conflicts, DebugCheckedUnwrap, QueryAccessError, ReadOnlyQueryData, ReleaseStateQueryData,
...
impl<'w> EntityWorldMut<'w> {
  ...
  pub fn contains_id(&self, component_id: ComponentId) -> bool {
    self
      .as_unsafe_entity_cell_readonly()
  ...
  pub unsafe fn into_components_mut_unchecked<Q: ReleaseStateQueryData + SingleEntityQueryData>(
    self,
  ...
  pub(crate) fn remove_with_requires_with_caller<T: Bundle>(
    ...
    let Some(mut remover) =
      (unsafe { BundleRemover::new_with_id(self.world, location.archetype_id, bundle_id, false) })
    else {
      return self;
    };
    // SAFETY:
  ...
  pub(crate) fn remove_by_id_with_caller(
    ...
    let bundle_id =
      self
        .world
        .bundles
        .init_component_info(&mut self.world.storages, components, component_id);
  ...
  pub(crate) fn remove_by_ids_with_caller<T: 'static>(
    ...
    let bundle_id =
      self
        .world
        .bundles
        .init_dynamic_info(&mut self.world.storages, components, component_ids);
  ...
  pub(crate) fn despawn_no_free_with_caller(&mut self, caller: MaybeLocation) {
    ...
    unsafe {
      ...
      deferred_world.trigger_on_remove(archetype, self.entity, archetype.iter_components(), caller);
    }
