...
impl SkinUniforms {
  ...
  pub fn skin_byte_offset(&self, skin: MainEntity) -> Option<SkinByteOffset> {
    self
      .skin_uniform_info
      .get(&skin)
      .map(|skin_uniform_info| SkinByteOffset::from_index(skin_uniform_info.offset() as usize))
  }
...
pub fn prepare_skins(
  ...
  let needed_size =
    (uniform.current_staging_buffer.len() as u64 + MAX_JOINTS as u64) * size_of::<Mat4>() as u64;
...
fn add_skin(
  ...
  for (joint_index, &joint) in skinned_mesh.joints.iter().enumerate() {
    ...
    let skinned_mesh_inverse_bindpose = skinned_mesh_inverse_bindposes
      .and_then(|skinned_mesh_inverse_bindposes| skinned_mesh_inverse_bindposes.get(joint_index));
