  ...
  for (camera_transform, camera, frustum, config, clusters, maybe_layers) in &mut views {
    ...
    if matches!(clusters.clusterable_objects, ClusterableObjects::Cpu(..)) {
      ...
      for clusterable_object in &clusterable_objects {
        ...
        let (clusterable_object_aabb_xy_ndc_z_view_min, clusterable_object_aabb_xy_ndc_z_view_max) =
        ...
        let clusterable_objects = match clusters.clusterable_objects {
          ...
          ClusterableObjects::Cpu(ref mut clusterable_objects_cpu) => clusterable_objects_cpu,
        };
        ...
        for z in min_cluster.z..=max_cluster.z {
          ...
          for y in min_cluster.y..=max_cluster.y {
            ...
            if y_center.is_none() || y != y_center.unwrap() {
              ...
              if let Some(projected) = project_to_plane_y(y_object, y_plane, is_orthographic) {
                y_object = projected;
              } else {
                continue;
              }
            ...
            match clusterable_object.object_type {
              ClusterableObjectType::SpotLight { .. } => {
                let (view_light_direction, angle_sin, angle_cos) = spot_light_dir_sin_cos.unwrap();
                for x in min_x..=max_x {
                  ...
                  if !angle_cull && !front_cull && !back_cull {
                    ...
                    clusterable_objects[cluster_index].add_spot_light(clusterable_object.entity);
              ...
              ClusterableObjectType::PointLight { .. } => {
                for _ in min_x..=max_x {
                  ...
                  clusterable_objects[cluster_index].add_point_light(clusterable_object.entity);
                  cluster_index += clusters.dimensions.z as usize;
                }
              ...
              ClusterableObjectType::Decal => {
                ...
                // decal's OBB.
                for _ in min_x..=max_x {
                  clusterable_objects[cluster_index].add_decal(clusterable_object.entity);
                  cluster_index += clusters.dimensions.z as usize;
                }
                total_cluster_index_count += (max_x - min_x + 1) as usize;
              }
