...
use bevy::{
  ...
  math::{vec3, vec4},
  pbr::{
    DefaultOpaqueRendererMethod, ExtendedMaterial, MaterialExtension, ScreenSpaceAmbientOcclusion,
...
fn move_camera(
  ...
  for mut camera_transform in cameras.iter_mut() {
    ...
    if theta_delta != 0.0 {
      camera_transform.translate_around(Vec3::ZERO, Quat::from_axis_angle(Vec3::Y, theta_delta));
...
fn handle_value_change_displayed_model(
  ...
  let Ok(radio::RadioButtonOptionValue(displayed_model)) = new_value_query.get(event.value) else {
    return;
  };
