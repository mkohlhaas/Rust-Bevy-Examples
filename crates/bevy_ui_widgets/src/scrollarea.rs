...
fn on_scroll_into_view(
  ...
  if let Ok((_target_node, target_transform, target_computed_node)) = q_node.get(scroll.entity) {
    ...
    let target_pos =
      target_affine.translation * target_computed_node.inverse_scale_factor - target_size * 0.5;
    ...
    let content_size =
      scroll_area_computed_node.content_size() * scroll_area_computed_node.inverse_scale_factor;
    ...
    if can_scroll_x {
      ...
      if target_local_top_left.x < view_min {
        ...
        scroll_pos.x = (target_local_bottom_right.x - scroll_area_size.x).clamp(0.0, max_range.x);
      }
    }
    ...
    if can_scroll_y {
      ...
      if target_local_top_left.y < view_min {
        ...
        scroll_pos.y = (target_local_bottom_right.y - scroll_area_size.y).clamp(0.0, max_range.y);
      }
    }
  }
}
