...
fn modify_selected_component(world: &mut World) {
  ...
  match selected {
    ...
    SelectedComponent::Sprite => {
      ...
      let downcast_sprite: &mut Sprite = reflected_component.downcast_mut::<Sprite>().unwrap();
    ...
    SelectedComponent::Transform => {
      ...
      let y_field: &mut dyn Reflect = y_field
        .try_as_reflect_mut()
        .expect("Found a dynamic type unexpectedly, but we need a concrete type to modify it");
