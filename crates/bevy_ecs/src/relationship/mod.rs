...
pub trait Relationship: Component + Sized {
  ...
  fn on_discard(
    ...
    if let Ok(mut target_entity_mut) = world.get_entity_mut(target_entity)
      && let Some(mut relationship_target) = target_entity_mut.get_mut::<Self::RelationshipTarget>()
    {
...
pub trait RelationshipTarget: Component<Mutability = Mutable> + Sized {
  ...
  fn with_capacity(capacity: usize) -> Self {
    let collection = <Self::Collection as RelationshipSourceCollection>::with_capacity(capacity);
...
impl MaybeRelationshipAccessor {
  ...
  pub fn accessor(&self) -> Option<&RelationshipAccessor> {
    match self {
      MaybeRelationshipAccessor::Accessor(relationship_accessor) => Some(relationship_accessor),
      _ => None,
    }
  }
