...
impl Plugin for VisibilityPlugin {
  fn build(&self, app: &mut bevy_app::App) {
    ...
    app
      .add_plugins(ValidateParentHasComponentPlugin::<InheritedVisibility>::default())
      ...
      .add_systems(
        ...
        (
          ...
          (check_visibility_cpu_culling, check_visibility_gpu_culling).in_set(CheckVisibility),
  ...
  for (entity, visibility, child_of, children) in &changed {
    ...
    if inherited_visibility.get() != is_visible {
      ...
      for &child in children.into_iter().flatten() {
        let _ = propagate_recursive(is_visible, child, &mut visibility_query, &children_query);
      }
    }
  }
