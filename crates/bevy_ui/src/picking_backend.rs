...
pub fn ui_picking(
  ...
  for (pointer_id, pointer_location) in pointers.iter().filter_map(|(pointer, pointer_location)| {
      ...
      for (pointer_id, cursor_position) in pointers_on_this_cam.iter() {
        if node.node.contains_point(*node.transform, *cursor_position)
          ...
          && let Some(target) = node
            ...
            .or_else(|| {
              (!settings.require_markers || node.pickable.is_some()).then_some(node_entity)
            })
        {
          hit_nodes
            ...
            .push((
              ...
              node.transform.inverse().transform_point2(*cursor_position) / node.node.size(),
            ));
        }
      }
    }
