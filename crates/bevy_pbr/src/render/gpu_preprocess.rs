...
use bevy_render::{
  batching::gpu_preprocessing::{
    ...
    GpuPreprocessingMode, GpuPreprocessingSupport, GpuUniformAllocationMetadata, IndirectBatchSet,
    ...
    PreprocessWorkItemBuffers, SceneUnpackingBuffers, SceneUnpackingBuffersKey, SceneUnpackingJob,
  ...
  render_resource::{
    ...
    SpecializedComputePipeline, SpecializedComputePipelines, TextureSampleType, UninitBufferVec,
  },
    ...
    render_app
      ...
      .add_systems(
        ...
        (
          ...
          (
            late_gpu_preprocess,
            late_prepass_build_indirect_parameters.run_if(
              any_match_filter::<(
                With<PreprocessBindGroups>,
                Without<SkipGpuPreprocess>,
                Without<NoIndirectDrawing>,
...
pub fn allocate_uniforms(
  ...
  let all_views = gather_shadow_cascades_for_view(view_entity, shadow_cascade_views, &light_query);
    ...
    for view_entity in all_views {
      ...
      for phase_type_id in batched_instance_buffers.phase_instance_buffers.keys() {
          ...
          .chain(phase_bin_unpacking_bind_groups.non_indexed.iter())
        {
          ...
          let workgroup_count =
            (bin_unpacking_bind_group.mesh_instance_count as usize).div_ceil(WORKGROUP_SIZE);
      ...
      &batched_instance_buffers.phase_instance_buffers
    {
      ...
      match *phase_bind_groups {
        PhasePreprocessBindGroups::Direct(ref bind_group) => {
          ...
          let PreprocessWorkItemBuffers::Direct(work_item_buffer) = work_item_buffers else {
            continue;
          };
    ...
    all_views.extend(
      shadow_cascade_views
        ...
        .filter(|light_entity| {
          light_query
            .get(**light_entity)
            .is_ok_and(|light_entity| matches!(*light_entity, LightEntity::Directional { .. }))
        })
        .copied(),
    );
  }
...
pub fn late_gpu_preprocess(
    ...
    &batched_instance_buffers.phase_instance_buffers
  {
    ...
    // Grab the work item buffers for this view.
    let Some(phase_work_item_buffers) = work_item_buffers.get(&view.retained_view_entity) else {
      continue;
    };
...
fn run_build_indirect_parameters(
  ...
  {
    let Some(phase_indirect_parameters_buffers) = indirect_parameters_buffers.get(phase_type_id)
    else {
      continue;
    };
    ...
    ComputePipelineDescriptor {
      label: Some(
        format!(
          "mesh preprocessing ({})",
          if key
            .contains(PreprocessPipelineKey::OCCLUSION_CULLING | PreprocessPipelineKey::EARLY_PHASE)
          {
...
impl FromWorld for PreprocessPipelines {
  fn from_world(world: &mut World) -> Self {
    ...
    let build_indirect_params_shader = load_embedded_asset!(world, "build_indirect_params.wgsl");
...
impl UniformAllocationPipelines {
  ...
  fn prepare(
    ...
    if self.local_scan.pipeline_id_local_scan.is_none() {
      self.local_scan.pipeline_id_local_scan = Some(
        uniform_allocation_local_scan_pipelines.specialize(pipeline_cache, &self.local_scan, ()),
      );
    }

    if self.global_scan.pipeline_id_global_scan.is_none() {
      self.global_scan.pipeline_id_global_scan = Some(
        uniform_allocation_global_scan_pipelines.specialize(pipeline_cache, &self.global_scan, ()),
      );
    }
    ...
    for (phase_type_id, phase_instance_buffers) in phase_instance_buffers {
      ...
      let (was_indirect, bind_group) = match *work_item_buffers {
        PreprocessWorkItemBuffers::Direct(ref work_item_buffer) => (
          false,
          preprocess_bind_group_builder.create_direct_preprocess_bind_groups(work_item_buffer),
        ),
        ...
        PreprocessWorkItemBuffers::Indirect {
          ...
          true,
          preprocess_bind_group_builder.create_indirect_occlusion_culling_preprocess_bind_groups(
            &view_depth_pyramids,
        ...
        PreprocessWorkItemBuffers::Indirect {
          ...
          true,
          preprocess_bind_group_builder.create_indirect_frustum_culling_preprocess_bind_groups(
            indexed_work_item_buffer,
            non_indexed_work_item_buffer,
          ),
        ),
...
impl<'a> PreprocessBindGroupBuilder<'a> {
    ...
    match (
      ...
      (
        ...
        let indexed_work_item_buffer_size = NonZero::<u64>::try_from(
          indexed_work_item_buffer.len() as u64 * u64::from(PreprocessWorkItem::min_size()),
        )
        .ok();
            ...
            &BindGroupEntries::with_indices((
              ...
              (
                ...
                BufferBinding {
                  ...
                  offset: 0,
                  size: NonZeroU64::new(late_indexed_indirect_parameters_buffer.size()),
                },
              ),
            )),
          ),
    ...
    match (
      ...
      (
        ...
        let non_indexed_work_item_buffer_size = NonZero::<u64>::try_from(
          non_indexed_work_item_buffer.len() as u64 * u64::from(PreprocessWorkItem::min_size()),
        )
        .ok();
            ...
            &BindGroupEntries::with_indices((
              ...
              (
                ...
                BufferBinding {
                  ...
                  offset: 0,
                  size: NonZeroU64::new(late_non_indexed_indirect_parameters_buffer.size()),
                },
              ),
            )),
          ),
    ...
    match (
      ...
      (
        ...
        let late_indexed_work_item_buffer_size = NonZero::<u64>::try_from(
          late_indexed_work_item_buffer.len() as u64 * u64::from(PreprocessWorkItem::min_size()),
        )
        .ok();
  ...
  fn create_indirect_frustum_culling_preprocess_bind_groups(
    ...
    Some(PhasePreprocessBindGroups::IndirectFrustumCulling {
      indexed: self.create_indirect_frustum_culling_indexed_bind_group(indexed_work_item_buffer),
      non_indexed: self
        .create_indirect_frustum_culling_non_indexed_bind_group(non_indexed_work_item_buffer),
    })
  }
        ...
        reset_non_indexed_indirect_batch_sets: phase_indirect_parameters_buffer
          ...
          .map(|non_indexed_batch_sets_buffer| {
            render_device.create_bind_group(
              ...
              &BindGroupEntries::sequential((non_indexed_batch_sets_buffer.as_entire_binding(),)),
            )
          }),
              ...
              &BindGroupEntries::with_indices((
                ...
                (
                  ...
                  BufferBinding {
                    ...
                    offset: 0,
                    size: NonZeroU64::new(
                      phase_indirect_parameters_buffer.indexed.batch_count() as u64
              ...
              &BindGroupEntries::with_indices((
                ...
                (
                  ...
                  BufferBinding {
                    ...
                    offset: 0,
                    size: NonZeroU64::new(
                      phase_indirect_parameters_buffer.non_indexed.batch_count() as u64
                        * size_of::<IndirectParametersMetadata>() as u64,
                    ),
                  },
                ),
...
fn create_bin_unpacking_bind_groups(
  ...
  let Some(bin_unpacking_metadata_buffer) = scene_unpacking_buffers.bin_unpacking_metadata.buffer()
  else {
    return;
  };
    ...
    uniform_allocation_bind_groups.insert(
      ...
      ViewPhaseUniformAllocationBindGroups {
        indexed: match phase_indirect_parameters_buffers.indexed.metadata_buffer() {
          None => vec![],
          Some(indexed_indirect_parameters_metadata_buffer) => view_phase_bin_unpacking_buffers
            .indexed_unpacking_jobs
            .iter()
            .map(|job| {
        ...
        {
          None => vec![],
          Some(non_indexed_indirect_parameters_metadata_buffer) => view_phase_bin_unpacking_buffers
            .non_indexed_unpacking_jobs
            .iter()
            .map(|job| {
              create_uniform_allocation_bind_group(
