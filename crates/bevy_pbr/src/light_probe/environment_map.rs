...
use crate::{
  add_cubemap_texture_view, binding_arrays_are_usable, RenderLightProbeFlags, MAX_VIEW_LIGHT_PROBES,
};
...
impl<'a> RenderViewEnvironmentMapBindGroupEntries<'a> {
  ...
  pub(crate) fn get(
    ...
    if binding_arrays_are_usable(render_device, render_adapter) {
      ...
      specular_texture_views.resize(MAX_VIEW_LIGHT_PROBES, &*fallback_image.cube.texture_view);
...
impl LightProbeComponent for EnvironmentMapLight {
  ...
  fn flags(
    ...
    if maybe_parallax_correction
      .is_some_and(|parallax_correction| !matches!(*parallax_correction, ParallaxCorrection::None))
    {
  ...
  fn create_render_view_light_probes(
    ...
    {
      render_view_light_probes.view_light_probe_info = Some(EnvironmentMapViewLightProbeInfo {
