...
use bevy_render::{
  ...
  prelude::*,
  render_asset::{prepare_assets, PrepareAssetError, RenderAsset, RenderAssetPlugin, RenderAssets},
  render_phase::{
    AddRenderCommand, BinnedPhaseItem, BinnedRenderPhasePlugin, BinnedRenderPhaseType,
    CachedRenderPipelinePhaseItem, DrawFunctionId, DrawFunctions, PhaseItem, PhaseItemBatchSetKey,
  ...
  fn render<'w>(
    ...
    match item.extra_index() {
      ...
      PhaseItemExtraIndex::IndirectParametersIndex {
        ...
        let Some(phase_indirect) = indirect_parameters_buffers.get(&TypeId::of::<P>()) else {
        ...
        let indirect_parameters_offset =
          indirect_parameters_range.start as u64 * size_of::<IndirectParametersNonIndexed>() as u64;
        ...
        match batch_set_index {
          Some(batch_set_index) => {
            let count_offset = u32::from(batch_set_index) * (size_of::<IndirectBatchSet>() as u32);
...
pub fn init_wireframe_3d_pipeline(
  ...
  let wide_bind_group_layout =
    render_device.create_bind_group_layout("wireframe_wide_bind_group_layout", &wide_bgl_entries);
...
pub fn specialize_wireframes(
  ...
  for (view, visible_entities) in &views {
    ...
    if dirty_wireframe_specializations.must_wipe_specializations_for_view(view.retained_view_entity)
    {
...
fn queue_wireframes(
  ...
  for (view, visible_entities) in &mut views {
    ...
    let view_pending_wireframe_queues = pending_wireframe_queues
      ...
      .expect("View pending wireframe queues should have been created in `specialize_wireframes`");
