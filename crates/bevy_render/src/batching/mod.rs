  ...
  fn new(item: &impl CachedRenderPipelinePhaseItem, user_data: T) -> Self {
    BatchSetMeta {
      ...
      dynamic_offset: match item.extra_index() {
        PhaseItemExtraIndex::DynamicOffset(dynamic_offset) => NonMaxU32::new(dynamic_offset),
        PhaseItemExtraIndex::None | PhaseItemExtraIndex::IndirectParametersIndex { .. } => None,
      },
      user_data,
    }
  }
