...
impl<D: QueryData, F: QueryFilter> QueryState<D, F> {
  ...
  pub unsafe fn new_unchecked(world: &mut World) -> Self {
    ...
    let mut state = unsafe { Self::from_states_uninitialized(world, fetch_state, filter_state) };
...
mod tests {
  ...
  fn can_transmute_changed() {
    ...
    let mut detection_query =
      QueryState::<(Entity, &A)>::new(&mut world).transmute_filtered::<Entity, Changed<A>>(&world);
