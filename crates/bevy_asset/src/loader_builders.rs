...
impl<'ctx, 'builder> NestedLoadBuilder<'ctx, 'builder> {
  ...
  pub fn load<'a, A: Asset>(self, path: impl Into<AssetPath<'a>>) -> Handle<A> {
    ...
    self
      .load_internal(TypeId::of::<A>(), Some(type_name::<A>()), path.into())
      .typed_debug_checked()
  }
  ...
  pub async fn load_value<'a, A: Asset>(
    ...
    self
      .load_typed_value_internal(path.into().into_owned(), None)
      .await
  }
  ...
  pub async fn load_erased_value<'a>(
    ...
    self
      .load_value_internal(Some(type_id), &path.into().into_owned(), None)
      .await
      .map(|(_, asset)| asset)
  }
  ...
  pub async fn load_untyped_value<'a>(
    ...
    self
      .load_value_internal(None, &path.into().into_owned(), None)
      .await
      .map(|(_, asset)| asset)
  }
  ...
  pub async fn load_value_from_reader<'a, A: Asset>(
    ...
    self
      .load_typed_value_internal(path.into().into_owned(), Some(reader))
      .await
  }
  ...
  pub async fn load_erased_value_from_reader<'a>(
    ...
    self
      .load_value_internal(Some(type_id), &path.into().into_owned(), Some(reader))
      .await
      .map(|(_, asset)| asset)
  }
  ...
  pub async fn load_untyped_value_from_reader<'a>(
    ...
    self
      .load_value_internal(None, &path.into().into_owned(), Some(reader))
      .await
      .map(|(_, asset)| asset)
  }
