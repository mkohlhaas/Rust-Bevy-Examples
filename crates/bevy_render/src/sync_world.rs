...
mod render_entities_world_query_impls {
  ...
  unsafe impl WorldQuery for RenderEntity {
    ...
    unsafe fn init_fetch<'w, 's>(
      ...
      unsafe { <&RenderEntity as WorldQuery>::init_fetch(world, component_id, last_run, this_run) }
    }
  ...
  unsafe impl WorldQuery for MainEntity {
    ...
    unsafe fn init_fetch<'w, 's>(
      ...
      unsafe { <&MainEntity as WorldQuery>::init_fetch(world, component_id, last_run, this_run) }
    }
  ...
  unsafe impl QueryData for MainEntity {
    ...
    unsafe fn fetch<'w, 's>(
      ...
      // SAFETY: defers to the `&T` implementation, with T set to `MainEntity`.
      let component = unsafe { <&MainEntity as QueryData>::fetch(state, fetch, entity, table_row) };
