  ...
  S: Scene,
{
  ...
  fn resolve(
    ...
    self
      .map(|value| value.resolve(context, scene))
      .unwrap_or(Ok(()))
  }
  ...
  L: SceneList,
{
  ...
  fn resolve_list(
    ...
    self
      .map(|scene_list| scene_list.resolve_list(context, scenes))
      .unwrap_or(Ok(()))
  }
...
pub trait PatchTemplate: Sized {
  ...
  fn patch_template<F: FnOnce(&mut Self, &mut ResolveContext)>(func: F) -> TemplatePatch<F, Self>;
}
...
impl<I: IntoObserverSystem<E, B, M> + Clone + Send + Sync, E: EntityEvent, B: Bundle, M: 'static>
