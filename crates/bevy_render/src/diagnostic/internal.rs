...
impl DiagnosticsRecorder {
  ...
  pub fn new(
    ...
    #[cfg(feature = "tracing-tracy")]
    let tracy_gpu_context = super::tracy_gpu::new_tracy_gpu_context(adapter_info, device, queue);
...
impl FrameData {
  fn new(
    ...
    let pipeline_statistics_query_set = if features.contains(Features::PIPELINE_STATISTICS_QUERY) {
    ...
    FrameData {
      ...
      num_timestamps: 0,
      supports_timestamps_inside_passes: features.contains(Features::TIMESTAMP_QUERY_INSIDE_PASSES),
  ...
  fn open_span(&mut self, pass_kind: Option<PassKind>, name: Cow<'static, str>) -> &mut SpanRecord {
      ...
      if let (Some(begin), Some(end)) = (span.begin_timestamp_index, span.end_timestamp_index) {
        ...
        #[cfg(feature = "tracing-tracy")]
        {
          ...
          if let Some(tracy_gpu_context) = &self.tracy_gpu_context {
            ...
            let mut tracy_gpu_span = tracy_gpu_context.span_alloc(name, "", "", 0).unwrap();
